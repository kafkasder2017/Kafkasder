...
import { generateDashboardInsights } from '../services/huggingfaceService';
    ...
    const channel = supabase
      ...
      .on<T>(
        ...
        payload => {
          ...
          setData(currentData =>
            currentData.filter(
              item => item.id !== (payload.old as Partial<T>).id
            )
          );
        }
      )
...
export const useDashboardData = () => {
  ...
  const refreshInsights = useCallback(
    async (currentStats?: DashboardStats) => {
      if (isAiLoading) return; // Prevent multiple simultaneous calls
      ...
      try {
        ...
        const insightsData = await generateDashboardInsights({
          ...
          activeProjects: statsToUse.activeProjects,
          monthlyDonations: statsToUse.monthlyDonations,
          totalMembers: statsToUse.totalMembers
        });
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const activeProjects = projects.filter(
        p => p.status === ProjeStatus._DEVAM_EDIYOR
      ).length;
      const pendingApplications = applications.filter(
        a =>
          a.durum === BasvuruStatus.BEKLEYEN ||
          a.durum === BasvuruStatus.INCELENEN
      ).length;
      ...
      // Recent Activities - memoized to prevent unnecessary re-renders
      ...
      const sortedApplications = [...applications]
        .sort(
          (a, b) =>
            new Date(b.basvuruTarihi).getTime() -
            new Date(a.basvuruTarihi).getTime()
        )
        .slice(0, 2);
      ...
      const activities: RecentActivity[] = [
        ...sortedDonations.map(d => ({
          ...
          amount: d.tutar.toLocaleString('tr-TR', {
            style: 'currency',
            currency: 'TRY'
          }),
      ...
      const monthlyDonationsAgg = donations.reduce(
        (acc, d) => {
  ...
  const memoizedData = useMemo(
    ...
    [stats, recentActivities, monthlyDonationData, insights]
  );
