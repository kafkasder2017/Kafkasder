...
import type { Dava, Durusma, Gelisime, Masraf } from '../types';
import { DavaStatus } from '../types';
...
const DavaDetay: React.FC = () => {
  ...
  const [modal, setModal] = useState<'durumsma' | 'gelisme' | 'masraf' | null>(
    null
  );
  ...
  const updateCaseOnServer = async (updatedCase: Dava) => {
    try {
      ...
      alert('Dava güncellenirken bir hata oluştu. Sayfa yenilenecek.');
      console.error(err);
  ...
  const handleAddGelisime = (newGelisime: Omit<Gelisime, 'id'>) => {
    ...
    const updatedCase = {
      ...caseDetails,
      gelismeler: [...(caseDetails.gelismeler || []), gelisimeToAdd].sort(
  ...
  const totalMasraf = useMemo(() => {
    return (
      caseDetails.masraflar?.reduce((acc, curr) => acc + curr.tutar, 0) || 0
    );
  }, [caseDetails.masraflar]);

  const getStatusClass = (status: DavaStatus) => {
    switch (status) {
      case DavaStatus._DEVAM_EDEN:
        return 'bg-blue-100 text-blue-800';
      case DavaStatus._SONUCLANAN:
        return 'bg-green-100 text-green-800';
      case DavaStatus._TEMYIZDE:
        ...
        <div className='flex justify-between items-start'>
          <div>
            <h2 className='text-2xl font-bold text-slate-800'>
              {caseDetails.davaKonusu}
            </h2>
            <p className='text-slate-500 mt-1'>
              {caseDetails.caseNumber} - {caseDetails.mahkeme}
            </p>
          </div>
          <span
            className={`px-3 py-1.5 text-sm font-semibold rounded-full ${getStatusClass(caseDetails.davaDurumu)}`}
          >
        ...
        <div className='lg:col-span-2 space-y-6'>
          ...
          <Card title='Gelişmeler' onAdd={() => setModal('gelisme')}>
            <ul className='space-y-4'>
              ...
              {!caseDetails.gelismeler?.length && (
                <p className='text-sm text-slate-400'>
                  Henüz bir gelişme kaydedilmemiş.
                </p>
              )}
            </ul>
          </Card>
          ...
          <Card title='Duruşmalar' onAdd={() => setModal('durumsma')}>
            <ul className='space-y-3'>
              ...
              {!caseDetails.durusmalar?.length && (
                <p className='text-sm text-slate-400'>
                  Henüz bir duruşma tarihi belirlenmemiş.
                </p>
              )}
            </ul>
          </Card>
          ...
          {/* Karar */}
          {caseDetails.davaDurumu === DavaStatus._SONUCLANAN && (
            <Card title='Mahkeme Kararı'>
              <p className='text-slate-600 whitespace-pre-wrap'>
                {caseDetails.karar || 'Karar bilgisi girilmemiş.'}
              </p>
            </Card>
          )}
        </div>
        ...
        <div className='space-y-6'>
          ...
          <Card title='Dava Bilgileri'>
            <ul className='space-y-2 text-sm'>
              ...
              <InfoRow
                ...
                value={caseDetails.sorumluAvukat}
              />
              <InfoRow label='Dava Türü' value={caseDetails.davaTuru} />
              <InfoRow
                label='Açılış Tarihi'
                value={new Date(caseDetails.acilisTarihi).toLocaleDateString(
                  'tr-TR'
                )}
              />
            </ul>
          ...
          <Card title='Masraflar' onAdd={() => setModal('masraf')}>
            ...
            <div className='border-t mt-3 pt-3 flex justify-between font-bold text-slate-800'>
              ...
              <span>
                {totalMasraf.toLocaleString('tr-TR', {
                  style: 'currency',
                  currency: 'TRY'
                })}
              </span>
          ...
          <Card title='Dava Dosyaları'>
            <p className='text-sm text-slate-400 text-center'>
              Dosya yönetimi entegrasyonu yakında.
            </p>
          </Card>
        </div>
      </div>
  ...
  <div className='bg-white p-6 rounded-lg shadow-sm'>
    <div className='flex justify-between items-center mb-4'>
      ...
      {onAdd && (
        <button
          onClick={onAdd}
          className='text-sm bg-blue-500 text-white px-3 py-1 rounded-md font-semibold hover:bg-blue-600 transition-colors flex items-center space-x-1'
        >
          <svg
...
// Modals
const DurusmaEkleModal: React.FC<{
  onClose: () => void;
  onSave: (d: Omit<Durusma, 'id'>) => void;
}> = ({ onClose, onSave }) => {
      ...
      <form onSubmit={handleSubmit} className='space-y-4'>
        ...
        <div>
          ...
          <textarea
            ...
            rows={3}
            className='mt-1 block w-full p-2 border border-slate-300 rounded-md'
            placeholder='Duruşma konusu, tanıklar vb.'
            required
          ></textarea>
        </div>
...
const GelisimeEkleModal: React.FC<{
  onClose: () => void;
  onSave: (g: Omit<Gelisime, 'id'>) => void;
}> = ({ onClose, onSave }) => {
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const formData = new FormData(e.currentTarget);
      ...
      <form onSubmit={handleSubmit} className='space-y-4'>
        <div>
          ...
          <textarea
            ...
            rows={4}
            className='mt-1 block w-full p-2 border border-slate-300 rounded-md'
            placeholder='Dava süreciyle ilgili yeni bir gelişme veya not ekleyin...'
            required
          ></textarea>
        </div>
        <div className='pt-4 flex justify-end space-x-3'>
...
const MasrafEkleModal: React.FC<{
  onClose: () => void;
  onSave: (m: Omit<Masraf, 'id'>) => void;
}> = ({ onClose, onSave }) => {
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const formData = new FormData(e.currentTarget);
      ...
      <form onSubmit={handleSubmit} className='space-y-4'>
        ...
        <div>
          ...
          <input
            ...
            name='aciklama'
            className='mt-1 block w-full p-2 border border-slate-300 rounded-md'
            placeholder='örn. Posta Gideri, Bilirkişi Ücreti'
            required
          />
        </div>
        <div className='pt-4 flex justify-end space-x-3'>
