...
import { getProjeler, createProje, updateProje, deleteProje } from '../services/apiService';
        ...
        <div>
            ...
            <div className="flex justify-between text-xs text-zinc-400 dark:text-zinc-500 mt-1">
             ...
             <div className="border-t border-zinc-200 dark:border-zinc-700 mt-4 pt-4 flex justify-end space-x-4">
                <ReactRouterDOM.Link to={`/projeler/${proje.id}`} className="text-sm font-semibold text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300">Detaylar</ReactRouterDOM.Link>
                <button onClick={() => onEdit(proje)} className="text-sm font-semibold text-yellow-600 dark:text-yellow-400 hover:text-yellow-800 dark:hover:text-yellow-300">Düzenle</button>
                <button onClick={() => onDelete(proje.id)} className="text-sm font-semibold text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-300">Sil</button>
            </div>
        </div>
    </div>
);
            ...
            <div className="space-y-6">
                <div className="flex flex-col md:flex-row items-center justify-between space-y-4 md:space-y-0 bg-white dark:bg-zinc-800 p-4 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-700">
                    <div className="flex items-center space-x-4">
                         <h3 className="text-lg font-semibold text-zinc-800 dark:text-zinc-200">Filtrele:</h3>
                         <select 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                     {filteredProjeler.map(proje => (
                        <ProjeCard key={proje.id} proje={proje} onEdit={handleEditClick} onDelete={handleDeleteClick} />
                    ))}
                </div>
                ...
                {filteredProjeler.length === 0 && (
                    <div className="text-center py-20 bg-white dark:bg-zinc-800 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-700 text-zinc-500 dark:text-zinc-400">
                        <svg xmlns="http://www.w3.org/2000/svg" className="mx-auto h-12 w-12 text-zinc-400 dark:text-zinc-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        <h3 className="mt-2 text-sm font-medium text-zinc-900 dark:text-zinc-200">Proje bulunamadı</h3>
                        <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Seçili filtreye uygun proje bulunamadı.</p>
                    </div>
                )}
            </div>
...
const ProjeFormModal: React.FC<{ proje: Partial<Proje>, onClose: () => void, onSave: (proje: Partial<Proje>) => void }> = ({ proje, onClose, onSave }) => {
    ...
    const handleGenerateWithAI = async () => {
        if (!formData.name) {
            setAiError('Lütfen önce bir proje adı girin.');
            return;
        }
        setIsGenerating(true);
        setAiError('');
        try {
            const result = await generateProjectPlan(formData.name);
            setFormData(prev => ({
                ...prev,
            ...
            setAiError(e.message || 'Plan oluşturulamadı.');
        } finally {
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="md:col-span-2">
                        <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300">Proje Adı</label>
                        <div className="relative">
                            <input type="text" name="name" value={formData.name || ''} onChange={handleChange} className="mt-1 block w-full p-2 border border-zinc-300 dark:border-zinc-600 rounded-lg pr-28 bg-white dark:bg-zinc-700" required />
                            <button 
                                type="button" 
                                onClick={handleGenerateWithAI}
                                disabled={isGenerating || !formData.name}
                                className="absolute top-1/2 right-1.5 -translate-y-1/2 bg-purple-100 text-purple-700 px-2 py-1 rounded-md text-xs font-semibold hover:bg-purple-200 disabled:bg-zinc-100 disabled:text-zinc-400 flex items-center space-x-1"
                            >
                                {isGenerating ? (
                    ...
                    <div>
                        <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300">Proje Yöneticisi</label>
                        <input type="text" name="manager" value={formData.manager || ''} onChange={handleChange} className="mt-1 block w-full p-2 border border-zinc-300 dark:border-zinc-600 rounded-lg bg-white dark:bg-zinc-700" required />
                    </div>
                    <div>
                        ...
                        <select name="status" value={formData.status || ''} onChange={handleChange} className="mt-1 block w-full p-2 border border-zinc-300 dark:border-zinc-600 rounded-lg bg-white dark:bg-zinc-700" required>
                             ...
                             {Object.values(ProjeStatus).map(s => <option key={s} value={s}>{s}</option>)}
                        </select>
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300">Başlangıç Tarihi</label>
                        <input type="date" name="startDate" value={formData.startDate || ''} onChange={handleChange} className="mt-1 block w-full p-2 border border-zinc-300 dark:border-zinc-600 rounded-lg bg-white dark:bg-zinc-700" required />
                    </div>
                     <div>
                        <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300">Bitiş Tarihi</label>
                        <input type="date" name="endDate" value={formData.endDate || ''} onChange={handleChange} className="mt-1 block w-full p-2 border border-zinc-300 dark:border-zinc-600 rounded-lg bg-white dark:bg-zinc-700" required />
                    </div>
                     <div>
                        <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300">Toplam Bütçe (TL)</label>
                        <input type="number" name="budget" value={formData.budget || ''} onChange={handleChange} className="mt-1 block w-full p-2 border border-zinc-300 dark:border-zinc-600 rounded-lg bg-white dark:bg-zinc-700" required />
                    </div>
                     <div>
                        <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300">Harcanan Tutar (TL)</label>
                        <input type="number" name="spent" value={formData.spent || ''} onChange={handleChange} className="mt-1 block w-full p-2 border border-zinc-300 dark:border-zinc-600 rounded-lg bg-white dark:bg-zinc-700" required />
                    </div>
                     <div className="md:col-span-2">
                    ...
                    <div className="md:col-span-2">
                        ...
                        <textarea name="description" value={formData.description || ''} onChange={handleChange} rows={5} className="mt-1 block w-full p-2 border border-zinc-300 dark:border-zinc-600 rounded-lg bg-white dark:bg-zinc-700" required />
                    </div>
                </div>
