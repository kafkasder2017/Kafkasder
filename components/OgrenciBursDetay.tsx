...
import type { OgrenciBursu, PerformansNotu, Odeme } from '../types';
...
const OgrenciBursDetay: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (bursData) {
          const paymentHistory = paymentsData.filter(
            p =>
              p.odemeTuru === OdemeTuru.BURS_ODEMESI &&
              p.kisi === bursData.ogrenciAdi
          );
  ...
  const [activeTab, setActiveTab] = useState<'payments' | 'performance'>(
    'payments'
  );
  const [isModalOpen, setIsModalOpen] = useState(false);

  const handleSavePerformanceNote = async (
    note: Omit<PerformansNotu, 'id'>
  ) => {
    if (!burs) return;
    ...
    const updatedBurs = {
      ...burs,
      performansGecmisi: updatedHistory,
      gpa: note.gpa
    };

    try {
      const saved = await updateOgrenciBursu(burs.id, {
        performansGecmisi: updatedHistory,
        gpa: note.gpa
      });
      setBurs(saved);
    } catch (err) {
      alert('Performans notu güncellenirken bir hata oluştu.');
    }
  ...
  return (
    <div className='space-y-6'>
      <div className='bg-white p-6 rounded-lg shadow-sm flex items-center space-x-6'>
        <img
          src={`https://i.pravatar.cc/100?u=${burs.ogrenciAdi}`}
          alt={burs.ogrenciAdi}
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6'>
        <StatCard
          ...
          icon={
            ...
            >
              <path d='m12 18-6-6 6-6 6 6Z' />
              <path d='m6 18-6-6 6-6 6 6Z' />
            </svg>
          }
        />
        <StatCard
          label='Aylık Tutar'
          value={burs.bursMiktari.toLocaleString('tr-TR', {
            style: 'currency',
            currency: 'TRY'
          })}
          icon={
        ...
        <StatCard
          label='Güncel GPA'
          value={typeof burs.gpa === 'number' ? burs.gpa.toFixed(2) : 'N/A'}
          icon={
            <svg
              xmlns='http://www.w3.org/2000/svg'
              width='24'
        ...
        <StatCard
          ...
          icon={
            <svg
              ...
              strokeLinejoin='round'
            >
              <path d='m12 12 4 10 4-10-4-10zM2 12h8' />
              <path d='M2 18h8' />
              <path d='M2 6h8' />
            </svg>
          }
      ...
      <div className='bg-white rounded-lg shadow-sm'>
        ...
        <div className='p-6'>
          {activeTab === 'payments' && (
            <PaymentHistory history={burs.odemeGecmisi || []} />
          )}
          {activeTab === 'performance' && (
            <PerformanceHistory
              history={burs.performansGecmisi || []}
  ...
  if (!history.length)
    return (
      <p className='text-sm text-slate-500'>
        Bu bursa ait ödeme kaydı bulunamadı.
      </p>
    );
  return (
    ...
    <div className='space-y-4'>
            ...
            <div className='flex justify-between items-center mb-1'>
              <p className='font-semibold text-slate-800'>
                ...
                <span className='text-blue-600'>
                  {typeof item.gpa === 'number' ? item.gpa.toFixed(2) : 'N/A'}
                </span>
              </p>
      ...
      {!history.length && (
        <p className='text-sm text-slate-500 text-center'>
          Henüz performans kaydı girilmemiş.
        </p>
      )}
    </div>
  );
...
const PerformansNotuModal: React.FC<{
  onClose: () => void;
  onSave: (note: Omit<PerformansNotu, 'id'>) => void;
  currentGpa: number;
}> = ({ onClose, onSave, currentGpa }) => {
      ...
      <form onSubmit={handleSubmit} className='space-y-4'>
        ...
        <div>
          ...
          <textarea
            ...
            rows={3}
            className='mt-1 block w-full p-2 border border-slate-300 rounded-md'
            placeholder='Dönem başarısı, devam durumu veya diğer notlar...'
            required
          ></textarea>
        </div>
