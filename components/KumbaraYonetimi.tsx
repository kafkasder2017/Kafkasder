...
import type { Kumbara, FinansalKayit } from '../types';
import {
  KumbaraStatus,
  KumbaraType,
  FinansalIslemTuru,
...
const getStatusClass = (status: KumbaraStatus) => {
  return status === KumbaraStatus.AKTIF
...
const KumbaraYonetimi: React.FC = () => {
  const {
    ...
    error,
    refresh
  } = useSupabaseQuery<Kumbara>('kumbaralar');

  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState<KumbaraStatus | 'all'>(
    'all'
  );
  ...
  const [editingKumbara, setEditingKumbara] = useState<Partial<Kumbara> | null>(
    null
  );
  ...
  const filteredKumbaralar = useMemo(() => {
    return (kumbaralar || [])
      .filter(kumbara => {
        ...
        const matchesStatus =
          statusFilter === 'all' || kumbara.status === statusFilter;
  ...
  const handleSaveBosaltma = async (
    kumbaraId: number,
        ...
        <div className='flex flex-col md:flex-row items-center justify-between space-y-4 md:space-y-0 mb-6'>
          ...
          <div className='flex items-center space-x-4'>
            <select
              ...
              value={typeFilter}
              onChange={e =>
                setTypeFilter(e.target.value as KumbaraType | 'all')
              }
            >
            ...
            <select
              ...
              value={statusFilter}
              onChange={e =>
                setStatusFilter(e.target.value as KumbaraStatus | 'all')
              }
            >
              ...
              {filteredKumbaralar.map(kumbara => (
                <tr key={kumbara.id} className='hover:bg-slate-50'>
                  ...
                  <td className='px-6 py-4'>
                    <a
                      href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(kumbara.location)}`}
                      target='_blank'
                  ...
                  <td className='px-6 py-4 font-semibold'>
                    {kumbara.balance.toLocaleString('tr-TR', {
                      style: 'currency',
                      currency: 'TRY'
                    })}
                  </td>
                  <td className='px-6 py-4'>
                    {kumbara.lastEmptied
                      ? new Date(kumbara.lastEmptied).toLocaleDateString(
                          'tr-TR'
                        )
                      : 'Hiç'}
                  </td>
                  <td className='px-6 py-4'>
                    <span
                      className={`px-2.5 py-1 text-xs font-semibold rounded-full ${getStatusClass(kumbara.status)}`}
                    >
                      {kumbara.status}
                    </span>
                  </td>
                  <td className='px-6 py-4 text-right'>
                    <div className='flex items-center justify-end space-x-2'>
                      <button
                        onClick={() => setQrModalKumbara(kumbara)}
                        className='text-purple-600 hover:text-purple-800 font-semibold'
                      >
                        QR Kod
                      </button>
                      <button
                        onClick={() => handleBosaltClick(kumbara)}
                        className='text-blue-600 hover:text-blue-800 font-semibold'
                      >
                        Boşalt
                      </button>
                      <button
                        onClick={() => handleEditClick(kumbara)}
                        className='text-yellow-600 hover:text-yellow-800 font-semibold'
                      >
                        Düzenle
                      </button>
                      <button
                        onClick={() => handleDeleteClick(kumbara.id)}
...
const KumbaraFormModal: React.FC<{
  kumbara: Partial<Kumbara>;
  onClose: () => void;
  onSave: (kumbara: Partial<Kumbara>) => void;
}> = ({ kumbara, onClose, onSave }) => {
  ...
  return (
    <Modal
      isOpen={true}
      onClose={onClose}
      title={isNew ? 'Yeni Kumbara Ekle' : 'Kumbara Bilgilerini Düzenle'}
    >
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
          ...
          <div>
            ...
            <select
              ...
              value={formData.type || ''}
              onChange={handleChange}
              className='mt-1 block w-full border border-slate-300 rounded-lg shadow-sm p-2 bg-white'
              required
            >
              <option value='' disabled>
                Seçiniz...
              </option>
              {Object.values(KumbaraType).map(type => (
          ...
          <div>
            ...
            >
              ...
              {Object.values(KumbaraStatus).map(status => (
...
const BosaltModal: React.FC<{
  kumbara: Kumbara;
  onClose: () => void;
  onSave: (kumbaraId: number, toplananTutar: number) => void;
}> = ({ kumbara, onClose, onSave }) => {
  ...
  return (
    <Modal
      isOpen={true}
      onClose={onClose}
      title={`Kumbara Boşalt: ${kumbara.code}`}
    >
      <form onSubmit={handleSubmit} className='space-y-4'>
        <p>
          <strong>Konum:</strong> {kumbara.location}
        </p>
        <p>
          Mevcut Bakiye:{' '}
        ...
        <div>
          ...
          <p className='text-xs text-slate-500 mt-1'>
            Bu tutar yeni bir gelir kaydı olarak işlenecek ve kumbara bakiyesi
            sıfırlanacaktır.
          </p>
        </div>
        <div className='pt-4 flex justify-end space-x-3'>
...
const QrCodeModal: React.FC<{ kumbara: Kumbara; onClose: () => void }> = ({
  kumbara,
  onClose
}) => {
    ...
    <Modal isOpen={true} onClose={onClose} title={`QR Kod: ${kumbara.code}`}>
      <div className='text-center'>
        <div ref={printAreaRef}>
          ...
          <img
            src={kumbara.qrCodeUrl}
            alt={`QR Code for ${kumbara.code}`}
