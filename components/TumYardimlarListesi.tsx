...
import type { Odeme, AyniYardimIslemi, Person, DepoUrunu } from '../types';
...
const TumYardimlarListesi: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError('');
      try {
        const [paymentsData, inKindData, peopleData, productsData] =
          await Promise.all([
            getOdemeler(),
            getAyniYardimIslemleri(),
            getPeople(),
  ...
  const birlesikYardimlar = useMemo((): BirlesikYardim[] => {
    const nakitYardimlar: BirlesikYardim[] = payments
      .filter(p =>
        [
          OdemeTuru.YARDIM_ODEMESI,
          OdemeTuru.BURS_ODEMESI,
          OdemeTuru.YETIM_DESTEGI,
          OdemeTuru.VEFA_DESTEGI
        ].includes(p.odemeTuru)
      )
      .map(p => ({
        id: `nakit-${p.id}`,
        kisiAdi: p.kisi,
        yardimTipi: 'Nakit',
        aciklama: p.aciklama,
        tutarMiktar: p.tutar.toLocaleString('tr-TR', {
          style: 'currency',
          currency: p.paraBirimi
        }),
        tarih: p.odemeTarihi,
        orjinalKayit: p
      }));
    ...
    return [...nakitYardimlar, ...ayniYardimlar].sort(
            ...
            {filteredYardimlar.map(yardim => (
              <tr key={yardim.id} className='hover:bg-slate-50'>
                <td className='px-6 py-4'>
                  {new Date(yardim.tarih).toLocaleDateString('tr-TR')}
                </td>
                ...
                <td className='px-6 py-4'>
                  <span
                    className={`px-2 py-1 text-xs font-semibold rounded-full ${yardim.yardimTipi === 'Nakit' ? 'bg-green-100 text-green-800' : 'bg-purple-100 text-purple-800'}`}
                  >
                    {yardim.yardimTipi}
                  </span>
                </td>
