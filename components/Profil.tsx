...
import type { Profil as ProfilData } from '../types';
...
const Profil: React.FC<ProfilProps> = ({ profile, onSave }) => {
  ...
  const FormCard: React.FC<{
    ...
    <div className='bg-white rounded-lg shadow-sm'>
      ...
      {footer && (
        <div className='bg-slate-50 px-6 py-3 rounded-b-lg flex justify-end'>
          {footer}
        </div>
      )}
    </div>
  ...
  return (
    <>
      <div className='max-w-4xl mx-auto space-y-6'>
        {status && (
          <div
            className={`p-4 rounded-md text-sm ${status.type === 'success' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}
          >
            {status.message}
          </div>
        )}
        ...
        >
          <div className='flex items-center space-x-6'>
            <img
              src={formData.profilFotoUrl}
              alt='Profil'
              className='h-20 w-20 rounded-full'
            />
            <p className='text-sm text-slate-500'>
              Profil fotoğrafınızı güncellemek için butona tıklayın.
            </p>
          </div>
        </FormCard>
...
const PhotoChangeModal: React.FC<{
  onClose: () => void;
      ...
      <form onSubmit={handleSubmit} className='space-y-4'>
        <div>
          ...
          <p className='text-xs text-slate-500 mt-1'>
            Lütfen yeni profil fotoğrafınızın URL'sini yapıştırın.
          </p>
        </div>
