...
import { getPeople, getAidatlarByUyeId, createAidat, updateAidat } from '../services/apiService';
import { generateAidatHatirlatma } from '../services/geminiService';
...
const UyeYonetimi: React.FC = () => {
    ...
    useEffect(() => {
        const fetchUyeler = async () => {
            ...
            try {
                ...
                const memberList = allPeople.filter(p => p.membershipType && p.membershipType !== MembershipType.GONULLU);
    ...
    const filteredUyeler = useMemo(() => {
        return uyeler.filter(uye => {
            ...
            const matchesStatus = statusFilter === 'all' || uye.durum === statusFilter;
            const matchesType = typeFilter === 'all' || uye.membershipType === typeFilter;
                    ...
                    <table className="w-full text-sm text-left text-slate-500">
                        <thead className="text-xs text-slate-700 uppercase bg-slate-50">
                            <tr>
                                ...
                                <th scope="col" className="px-6 py-4 font-semibold">Üyelik Tipi</th>
                        ...
                        <tbody className="divide-y divide-slate-200">
                            {filteredUyeler.map(uye => (
                                <tr key={uye.id} className="hover:bg-slate-50">
                                    <td className="px-6 py-4 font-medium text-slate-900">{uye.ad} {uye.soyad}</td>
                                    <td className="px-6 py-4">{uye.membershipType}</td>
                                    <td className="px-6 py-4">{new Date(uye.kayitTarihi).toLocaleDateString('tr-TR')}</td>
                                    <td className="px-6 py-4">
                                        <span className={`px-2.5 py-1 text-xs font-semibold rounded-full ${getStatusClass(uye.durum)}`}>
                                            {uye.durum}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4 text-right">
                                        <button onClick={() => setSelectedUye(uye)} className="text-blue-600 hover:text-blue-800 font-semibold">
                                            Aidat Takibi
                                        </button>
                                    </td>
                                </tr>
...
const AidatTakipModal: React.FC<{ uye: Person; onClose: () => void; }> = ({ uye, onClose }) => {
    ...
    const handleMarkAsPaid = async (aidat: Aidat) => {
        try {
            const updated = await updateAidat(aidat.id, { durum: AidatDurumu.ODENDI, odemeTarihi: new Date().toISOString().split('T')[0] });
    ...
    const handleGenerateReminder = async (aidat: Aidat) => {
        ...
        setReminderText('');
        try {
            const text = await generateAidatHatirlatma(uye.ad, aidat.donem, aidat.tutar);
                    ...
                    {!isLoading && !error && aidatlar.map(aidat => (
                        <div key={aidat.id} className="p-3 bg-slate-50 rounded-md">
                            <div className="flex justify-between items-center">
                                <div>
                                    <p className="font-semibold text-slate-700">{aidat.donem}</p>
                                    <p className="text-sm font-bold text-slate-800">{aidat.tutar.toLocaleString('tr-TR', {style:'currency', currency: 'TRY'})}</p>
                                </div>
                                {aidat.durum === AidatDurumu.ODENDI ? (
                                    <span className="text-xs font-bold text-green-600 bg-green-100 px-2 py-1 rounded-full">ÖDENDİ</span>
                                ) : (
                                    <button onClick={() => handleMarkAsPaid(aidat)} className="text-xs bg-yellow-100 text-yellow-800 font-bold px-2 py-1 rounded-full hover:bg-yellow-200">ÖDENDİ İŞARETLE</button>
                                )}
                            </div>
                             {aidat.durum === AidatDurumu.BEKLEMEDE && (
                                <div className="mt-2 pt-2 border-t">
                                    <button onClick={() => handleGenerateReminder(aidat)} disabled={isGeneratingReminder} className="text-xs text-purple-600 font-semibold hover:underline disabled:text-slate-400">
                                        {isGeneratingReminder ? 'Oluşturuluyor...' : 'AI ile Hatırlatma Oluştur'}
                                    </button>
                                    {reminderText && <p className="text-xs bg-purple-50 p-2 mt-2 rounded-md font-mono">{reminderText}</p>}
                                </div>
                            )}
                        </div>
                    ))}
                    {!isLoading && !error && aidatlar.length === 0 && <p className="text-sm text-slate-500">Aidat kaydı bulunamadı.</p>}
                    </div>
                </div>
                {/* Yeni Aidat Ekle */}
                <form onSubmit={handleCreateAidat} className="space-y-3 p-4 bg-slate-50 rounded-lg self-start">
                    <fieldset disabled={!!error}>
                        <h4 className="font-semibold text-slate-800">Yeni Aidat Oluştur</h4>
                        <div>
                            <label className="text-sm font-medium text-slate-600">Dönem</label>
                            <input type="month" value={newAidat.donem} onChange={e => setNewAidat(p => ({...p, donem: e.target.value}))} className="w-full mt-1 p-2 border border-slate-300 rounded-lg" required />
                        </div>
                        <div>
                            <label className="text-sm font-medium text-slate-600">Tutar (TL)</label>
                            <input type="number" step="0.01" value={newAidat.tutar} onChange={e => setNewAidat(p => ({...p, tutar: e.target.value}))} className="w-full mt-1 p-2 border border-slate-300 rounded-lg" required />
                        </div>
                        <button type="submit" disabled={!!error} className="w-full bg-blue-600 text-white p-2 rounded-lg font-semibold hover:bg-blue-700 disabled:bg-slate-400 disabled:cursor-not-allowed">Oluştur</button>
                    </fieldset>
                </form>
            </div>
