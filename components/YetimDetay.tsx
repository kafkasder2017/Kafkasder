...
import type { Yetim, SaglikNotu, EgitimNotu, Odeme } from '../types';
import { OdemeTuru } from '../types';
...
const YetimDetay: React.FC = () => {
  ...
  const supportHistory = useMemo(() => {
    ...
    return payments.filter(
      o => o.kisi === yetim.veliAdi && o.odemeTuru === OdemeTuru.YETIM_DESTEGI
    );
  }, [yetim, payments]);
  ...
  const handleSaveEducationNote = (
    note: Omit<EgitimNotu, 'id' | 'yetimId'>
  ) => {
    if (!yetim) return;
    const noteToAdd: EgitimNotu = {
      ...note,
      id: Date.now(),
      yetimId: yetim.id
    };
  ...
  return (
    <div className='space-y-6'>
      <div className='bg-white p-6 rounded-lg shadow-sm flex items-center space-x-6'>
        <img
          src={`https://i.pravatar.cc/100?u=${yetim.adiSoyadi}`}
          alt={yetim.adiSoyadi}
        ...
        <div>
          ...
          <p className='text-slate-500 mt-1'>
            Veli: {yetim.veliAdi} ({yetim.veliTelefonu})
          </p>
        </div>
      </div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6'>
        <StatCard
          label='Yaş'
          value={`${calculateAge(yetim.dogumTarihi)} (${yetim.cinsiyet})`}
          icon={
            <svg
            ...
            >
              <path d='M12 2a5 5 0 0 0-5 5c0 1.5.6 2.8 1.5 3.8A5.4 5.4 0 0 0 12 14a5.4 5.4 0 0 0 3.5-1.2c.9-1 1.5-2.3 1.5-3.8A5 5 0 0 0 12 2Z' />
            </svg>
          }
        />
        <StatCard
        ...
        <StatCard
          ...
          icon={
            ...
            >
              <path d='M7 10v12' />
              <path d='M15 5.88 14 10h4.83a2 2 0 0 1 1.94 2.53l-1.38 6.14a2 2 0 0 1-1.94 1.47H4a2 2 0 0 1-2-2V7a2 2 0 0 1 2-2h2.5' />
              <path d='M18 9.12V7a2 2 0 0 0-2-2H8.5' />
            </svg>
          }
        />
      </div>
      ...
      <div className='bg-white rounded-lg shadow-sm'>
        ...
        <div className='p-6'>
          {activeTab === 'support' && (
            <SupportHistory history={supportHistory} />
          )}
          {activeTab === 'health' && (
            <HealthHistory
              history={yetim.saglikGecmisi || []}
              onAddNote={() => setModal('health')}
            />
          )}
          {activeTab === 'education' && (
            <EducationHistory
              history={yetim.egitimGecmisi || []}
              onAddNote={() => setModal('education')}
            />
          )}
        </div>
      </div>
      ...
      {modal === 'health' && (
        <HealthNoteModal
          ...
          onSave={handleSaveHealthNote}
        />
      )}
      {modal === 'education' && (
        <EducationNoteModal
          onClose={() => setModal(null)}
          onSave={handleSaveEducationNote}
        />
      )}
    </div>
  );
  ...
  if (!history.length)
    return (
      <p className='text-sm text-slate-500'>
        Bu yetime ait destek ödemesi kaydı bulunamadı.
      </p>
    );
  return (
...
const NoteTakingSection: React.FC<{
  title: string;
  history: Array<{ id: number; tarih: string; not: string }>;
  onAddNote: () => void;
  noItemText: string;
}> = ({ title, history, onAddNote, noItemText }) => {
  return (
    <div className='space-y-4'>
      ...
      {!history.length && (
        <p className='text-sm text-slate-500 text-center py-4'>{noItemText}</p>
      )}
    </div>
  );
};
...
const HealthHistory: React.FC<{
  ...
  <NoteTakingSection
    ...
    history={history}
    onAddNote={onAddNote}
    noItemText='Henüz sağlık kaydı girilmemiş.'
  />
);
...
const EducationHistory: React.FC<{
  ...
  <NoteTakingSection
    ...
    history={history}
    onAddNote={onAddNote}
    noItemText='Henüz eğitim kaydı girilmemiş.'
  />
);

const HealthNoteModal: React.FC<{
  onClose: () => void;
  onSave: (note: Omit<SaglikNotu, 'id' | 'yetimId'>) => void;
}> = ({ onClose, onSave }) => {
      ...
      <form onSubmit={handleSubmit} className='space-y-4'>
        ...
        <div>
          ...
          <textarea
            ...
            rows={4}
            className='mt-1 block w-full p-2 border border-slate-300 rounded-md'
            placeholder='Kontrol, hastalık, ilaç vb. bilgiler'
            required
          />
        </div>
...
const EducationNoteModal: React.FC<{
  onClose: () => void;
  onSave: (note: Omit<EgitimNotu, 'id' | 'yetimId'>) => void;
}> = ({ onClose, onSave }) => {
  const [formData, setFormData] = useState({
    tarih: new Date().toISOString().split('T')[0],
    not: ''
      ...
      <form onSubmit={handleSubmit} className='space-y-4'>
        ...
        <div>
          ...
          <textarea
            ...
            rows={4}
            className='mt-1 block w-full p-2 border border-slate-300 rounded-md'
            placeholder='Okul başarısı, öğretmen notu, özel ihtiyaçlar vb.'
            required
          />
        </div>
        <div className='pt-4 flex justify-end space-x-3'>
