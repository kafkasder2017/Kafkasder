...
import type { Gonullu, Person, EtkinlikKatilimi, Proje, Gorev } from '../types';
import { Beceri } from '../types';
...
const GonulluDetay: React.FC = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (!gonulluData.personId) {
          throw new Error(
            `Gönüllü kaydı (ID: ${gonulluData.id}) ilişkili bir kişi ID'sine sahip değil.`
          );
        }
        ...
        if (!personData) {
          throw new Error(
            `Gönüllüye bağlı kişi kaydı (ID: ${gonulluData.personId}) bulunamadı.`
          );
        }
  ...
  const onUpdateGonullu = async (updatedGonullu: Gonullu) => {
    try {
      ...
      alert('Gönüllü güncellenirken bir hata oluştu.');
    }
  };
  ...
  return (
    <div className='space-y-6'>
      <div className='bg-white p-6 rounded-lg shadow-sm flex items-center space-x-6'>
        ...
        <div>
          ...
          <p className='text-slate-500 mt-1'>
            Gönüllü (Başlangıç:{' '}
            {new Date(gonullu.baslangicTarihi).toLocaleDateString('tr-TR')})
          </p>
        </div>
      </div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6'>
        ...
        <StatCard
          label='Toplam Katkı'
          value={`${gonullu.toplamSaat || 0} saat`}
          icon={
            <svg
              xmlns='http://www.w3.org/2000/svg'
              width='24'
        ...
        <StatCard
          ...
          value={`${gonullu.etkinlikGecmisi?.length || 0} adet`}
          icon={
            <svg
              xmlns='http://www.w3.org/2000/svg'
              width='24'
          ...
          <div className='bg-white p-6 rounded-lg shadow-sm'>
            ...
            <div className='space-y-2 text-sm'>
              <p className='text-slate-600'>
                <strong>Telefon:</strong> {person.cepTelefonu}
              </p>
              <p className='text-slate-600'>
                <strong>E-posta:</strong> {person.email || 'Belirtilmemiş'}
              </p>
              <p className='text-slate-600'>
                <strong>Adres:</strong> {person.adres}, {person.yerlesim},{' '}
                {person.sehir}
              </p>
            </div>
          </div>
...
const ActivityHistory: React.FC<{ history: EtkinlikKatilimi[] }> = ({
  history
}) => {
  if (!history.length)
    return (
      <p className='text-sm text-slate-500 text-center py-4'>
        Etkinlik geçmişi bulunmamaktadır.
      </p>
    );
  return (
...
const AssignedTasks: React.FC<{
  tasks: (Gorev & { projectName: string })[];
}> = ({ tasks }) => {
  if (!tasks.length)
    return (
      <p className='text-sm text-slate-500 text-center py-4'>
        Atanmış aktif bir görev bulunmamaktadır.
      </p>
    );
  return (
    <ul className='space-y-3'>
        ...
        <li key={task.id} className='p-3 bg-slate-50 rounded-md'>
          <div className='flex justify-between items-start'>
            <div>
              ...
              <p className='text-xs text-purple-600 font-semibold bg-purple-100 px-1.5 py-0.5 rounded-full inline-block mt-1'>
                {task.projectName}
              </p>
            </div>
            <span
              className={`px-2 py-0.5 text-xs font-medium rounded-full ${task.durum === 'Tamamlandı' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}
            >
              {task.durum}
            </span>
          </div>
...
const ActivityFormModal: React.FC<{
  onClose: () => void;
  onSave: (activity: Omit<EtkinlikKatilimi, 'id'>) => void;
}> = ({ onClose, onSave }) => {
