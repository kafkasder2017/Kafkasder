...
import type { DenetimKaydi } from '../types';
import { LogAction, LogEntityType } from '../types';
...
const getEntityIcon = (entityType: LogEntityType) => {
  const iconStyle =
    'w-5 h-5 inline-block mr-2 text-zinc-500 dark:text-zinc-400';
  switch (entityType) {
    case LogEntityType._PERSON:
      return <span className={iconStyle}>{ICONS.PEOPLE}</span>;
    case LogEntityType._APPLICATION:
      return <span className={iconStyle}>{ICONS.AID_RECIPIENT}</span>;
    case LogEntityType._PROJECT:
      return <span className={iconStyle}>{ICONS.CALENDAR}</span>;
    case LogEntityType._DONATION:
      return <span className={iconStyle}>{ICONS.DONATION}</span>;
    case LogEntityType._USER:
      return <span className={iconStyle}>{ICONS.PEOPLE}</span>;
    case LogEntityType._VEFA:
      return <span className={iconStyle}>{ICONS.HEART_HAND}</span>;
    case LogEntityType._YETIM:
      return <span className={iconStyle}>{ICONS.ORPHAN}</span>;
    case LogEntityType._BURS:
      return <span className={iconStyle}>{ICONS.SCHOLARSHIP}</span>;
    case LogEntityType._SYSTEM:
      return <span className={iconStyle}>{ICONS.SETTINGS}</span>;
    case LogEntityType._COMMENT:
      return <span className={iconStyle}>{ICONS.MESSAGE}</span>;
    default:
      return null;
  }
};
      ...
      <div className='space-y-4 mb-6 p-4 bg-zinc-50 dark:bg-zinc-800/50 rounded-lg border border-zinc-200 dark:border-zinc-700'>
        <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
          ...
          >
            ...
            {Object.values(LogEntityType).map(e => (
        ...
        <div className='flex flex-col md:flex-row items-center gap-4 flex-wrap'>
          <div className='flex items-center gap-2'>
            <input
              ...
              onChange={handleFilterChange}
              className='px-3 py-2 border border-zinc-300 dark:border-zinc-600 rounded-lg bg-white dark:bg-zinc-700 text-sm'
            />
          ...
          <div className='flex items-center gap-2'>
            <button
              ...
              className='text-sm px-3 py-1.5 border border-zinc-300 dark:border-zinc-600 rounded-md hover:bg-zinc-100 dark:hover:bg-zinc-700'
            >
              Bugün
            </button>
            <button
          ...
          <button
            ...
            className='text-sm px-4 py-2 text-blue-600 font-semibold hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-md ml-auto'
          >
            Filtreleri Temizle
          </button>
        </div>
            ...
            {filteredLogs.map(log => (
              ...
              >
                ...
                <td className='px-6 py-4'>
                  <span
                    className={`px-2.5 py-1 text-xs font-semibold rounded-full ${getActionClass(log.eylem)}`}
                  >
                    {log.eylem}
                  </span>
                </td>
                <td className='px-6 py-4'>
                  <div className='flex items-center'>
                    ...
                    <span>
                      {log.entityTipi} {log.entityId && `(#${log.entityId})`}
                    </span>
                  </div>
                </td>
