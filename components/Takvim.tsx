...
import type { CalendarEvent, Etkinlik, Proje, Dava } from '../types';
...
const CalendarHeader: React.FC<{
  ...
  <div className='flex items-center justify-between mb-4'>
    <div className='flex items-center space-x-2'>
      <button
        onClick={onToday}
        className='px-4 py-2 text-sm font-semibold text-zinc-700 dark:text-zinc-200 bg-white dark:bg-zinc-700 border border-zinc-300 dark:border-zinc-600 rounded-md hover:bg-zinc-50 dark:hover:bg-zinc-600'
      >
        Bugün
      </button>
      <button
        onClick={onPrevMonth}
        className='p-2 rounded-md hover:bg-zinc-100 dark:hover:bg-zinc-700'
      >
        {ICONS.CHEVRON_DOWN &&
  ...
  return (
    <div className='grid grid-cols-7'>
      {days.map(day => (
        <div
          key={day}
          className='text-center text-sm font-semibold text-zinc-500 dark:text-zinc-400 py-2 border-b border-zinc-200 dark:border-zinc-700'
        >
          {day}
        </div>
      ))}
...
const getEventTypeClass = (type: CalendarEventType) => {
  switch (type) {
    case CalendarEventType._ETKINLIK:
      return 'bg-blue-500 hover:bg-blue-600';
    case CalendarEventType._GOREV:
      return 'bg-yellow-500 hover:bg-yellow-600';
    case CalendarEventType._DURUSMA:
...
const CalendarEventModal: React.FC<{
  ...
  <Modal isOpen={true} onClose={onClose} title={event.title}>
    <div className='space-y-4'>
      <div>
        <span
          className={`px-2 py-1 text-xs font-semibold rounded-full text-white ${getEventTypeClass(event.type)}`}
        >
          {event.type}
        </span>
      </div>
      ...
      <p>
        <strong>Detaylar:</strong> {event.details}
      </p>
      ...
      try {
        ...
        etkinlikler.forEach((e: Etkinlik) => {
          transformedEvents.push({
            ...
            title: e.ad,
            date: e.tarih,
            type: CalendarEventType._ETKINLIK,
        ...
        davalar.forEach((d: Dava) => {
          (d.durusmalar || []).forEach(dur => {
            transformedEvents.push({
              ...
              title: d.davaKonusu,
              date: dur.tarih,
              type: CalendarEventType._DURUSMA,
      ...
      <div className='bg-white dark:bg-zinc-800 p-6 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-700 h-full flex flex-col'>
        ...
        <div className='grid grid-cols-7 grid-rows-6 flex-1'>
          {blanks.map((_, i) => (
            <div
              key={`blank-${i}`}
              className='border-r border-b border-zinc-200 dark:border-zinc-700'
            ></div>
          ))}
          {days.map(day => {
            const isToday =
            ...
            return (
              <div
                key={day}
                className='relative p-2 border-r border-b border-zinc-200 dark:border-zinc-700'
              >
                <span
                  className={`absolute top-2 right-2 text-sm font-semibold ${isToday ? 'bg-blue-600 text-white rounded-full w-6 h-6 flex items-center justify-center' : 'text-zinc-700 dark:text-zinc-300'}`}
                >
                  {day}
                </span>
