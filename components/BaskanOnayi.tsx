...
import { BasvuruStatus, BasvuruOncelik } from '../types';
...
export const BaskanOnayi: React.FC = () => {
  ...
  const applicationsToReview = useMemo(() => {
    return applications.filter(
      app =>
        app.durum === BasvuruStatus.ONAYLANAN &&
        !app.baskanOnayi &&
        app.baskanOnayNotu === null
    );
  }, [applications]);
    ...
    <>
      <div className='bg-white p-6 rounded-xl shadow-sm'>
        <div className='border-b pb-4 mb-4'>
          <h2 className='text-xl font-bold text-slate-800'>
            Başkan Onayı Bekleyen Başvurular
          </h2>
          <p className='text-sm text-slate-500 mt-1'>
            Komisyon tarafından onaylanmış ve nihai kararınızı bekleyen yardım
            başvuruları.
          </p>
        </div>
        ...
        {applicationsToReview.length > 0 ? (
          <div className='space-y-4'>
            {applicationsToReview.map(app => (
              <div
                key={app.id}
                className={`p-4 bg-slate-50 rounded-lg border-l-4 transition-all duration-200 hover:shadow-md ${getPriorityClass(app.oncelik)}`}
              >
                <div className='grid grid-cols-1 md:grid-cols-5 gap-4 items-center'>
                  {/* Info */}
                  <div className='md:col-span-2'>
                    <p className='font-bold text-slate-800'>
                      {peopleMap.get(app.basvuruSahibiId) || 'Bilinmeyen Kişi'}
                    </p>
                    <p className='text-sm text-slate-600'>{app.basvuruTuru}</p>
                    <p className='text-xs text-slate-400'>
                      Başvuru Tarihi:{' '}
                      {new Date(app.basvuruTarihi).toLocaleDateString('tr-TR')}
                    </p>
                  </div>
                  {/* Tutar */}
                  <div className='text-slate-700'>
                    ...
                    <p className='font-semibold text-lg'>
                      {app.talepTutari.toLocaleString('tr-TR', {
                        style: 'currency',
                        currency: 'TRY'
                      })}
                    </p>
                  ...
                  <div className='text-slate-700'>
                    ...
                    <p className='text-sm italic'>
                      "{app.degerlendirmeNotu || 'Not belirtilmemiş.'}"
                    </p>
                  </div>
                  {/* Actions */}
                  <div className='flex items-center justify-end space-x-3'>
                    ...
                    <button
                      onClick={() =>
                        setOnayModal({ application: app, isApprove: false })
                      }
                    ...
                    <button
                      onClick={() =>
                        setOnayModal({ application: app, isApprove: true })
                      }
          ...
          <div className='text-center py-20 text-slate-500'>
            ...
            <p className='mt-1 text-sm text-slate-500'>
              Onayınızı bekleyen yeni bir başvuru bulunmamaktadır.
            </p>
          </div>
        )}
      </div>
      {onayModal && (
        <OnayModal
          ...
          applicantName={
            peopleMap.get(onayModal.application.basvuruSahibiId) ||
            'Bilinmeyen Kişi'
          }
    ...
    >
      <form onSubmit={handleSubmit} className='space-y-4'>
        <p>
          ...
          <span
            className={`font-bold ${isApprove ? 'text-green-600' : 'text-red-600'}`}
          >
        ...
        <div className='pt-4 flex justify-end space-x-3'>
          ...
          <button
            type='submit'
            className={`text-white px-4 py-2 rounded-lg font-semibold ${isApprove ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'}`}
          >
