...
import type { OgrenciBursu } from '../types';
import { BursTuru, BursDurumu } from '../types';

import Modal from './Modal';

const getStatusClass = (status: BursDurumu) => {
  switch (status) {
    case BursDurumu.AKTIF:
      return 'bg-green-100 text-green-800';
    case BursDurumu.TAMAMLANDI:
      return 'bg-blue-100 text-blue-800';
    case BursDurumu.IPTAL_EDILDI:
...
const OgrenciBurslari: React.FC = () => {
  ...
  const [editingBurs, setEditingBurs] = useState<Partial<OgrenciBursu> | null>(
    null
  );
  ...
  const filteredBurslar = useMemo(() => {
    return scholarships.filter(burs => {
      const matchesSearch =
        burs.ogrenciAdi.toLowerCase().includes(searchTerm.toLowerCase()) ||
        burs.okulAdi.toLowerCase().includes(searchTerm.toLowerCase());
      const matchesStatus =
        statusFilter === 'all' || burs.durum === statusFilter;
  ...
  const handleSaveBurs = async (bursToSave: Partial<OgrenciBursu>) => {
    try {
      if (bursToSave.id) {
        ...
        setScholarships(
          scholarships.map(b => (b.id === updated.id ? updated : b))
        );
      } else {
        const created = await createOgrenciBursu(
          bursToSave as Omit<OgrenciBursu, 'id'>
        );
        ...
        <div className='flex flex-col md:flex-row items-center justify-between space-y-4 md:space-y-0 mb-6'>
          ...
          <div className='flex items-center space-x-4'>
            ...
            >
              ...
              {Object.values(BursTuru).map(tur => (
            ...
            <select
              ...
              value={statusFilter}
              onChange={e =>
                setStatusFilter(e.target.value as BursDurumu | 'all')
              }
            >
              <option value='all'>Tüm Durumlar</option>
              {Object.values(BursDurumu).map(durum => (
              ...
              {filteredBurslar.map(burs => (
                <tr key={burs.id} className='hover:bg-slate-50'>
                  ...
                  <td className='px-6 py-4 font-semibold'>
                    {burs.bursMiktari.toLocaleString('tr-TR', {
                      style: 'currency',
                      currency: 'TRY'
                    })}
                  </td>
                  <td className='px-6 py-4'>
                    {typeof burs.gpa === 'number' ? burs.gpa.toFixed(2) : 'N/A'}
                  </td>
                  <td className='px-6 py-4 text-xs'>
                    {new Date(burs.baslangicTarihi).toLocaleDateString('tr-TR')}{' '}
                    - {new Date(burs.bitisTarihi).toLocaleDateString('tr-TR')}
                  </td>
                  <td className='px-6 py-4'>
                    <span
                      className={`px-2.5 py-1 text-xs font-semibold rounded-full ${getStatusClass(burs.durum)}`}
                    >
                      {burs.durum}
                    </span>
                  </td>
...
const BursFormModal: React.FC<{
  burs: Partial<OgrenciBursu>;
  onClose: () => void;
  onSave: (burs: Partial<OgrenciBursu>) => void;
}> = ({ burs, onClose, onSave }) => {
  ...
  return (
    <Modal
      isOpen={true}
      onClose={onClose}
      title={isNew ? 'Yeni Burs Başvurusu Ekle' : 'Burs Bilgilerini Düzenle'}
    >
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
          ...
          <div>
            ...
            >
              ...
              {Object.values(BursTuru).map(type => (
          ...
          <div>
            ...
            >
              ...
              {Object.values(BursDurumu).map(status => (
