...
import { DosyaSistemiOgesi, Klasor, Dosya, FileType } from '../types';
import { listFiles, uploadFile, deleteFiles, createFolder as apiCreateFolder, updateFile } from '../services/apiService';
        ...
        if (file) {
            ...
            try {
                ...
                if (newFile.fileType === 'image') {
                    ...
                    if (tags.length > 0) {
                        const updatedFileWithTags = await updateFile(newFile.id, { tags });
                        setItems(prev => prev.map(item => item.id === newFile.id ? updatedFileWithTags : item));
                    }
                                    ...
                                    <div className="aspect-w-1 aspect-h-1 w-full bg-zinc-100 dark:bg-zinc-700/50 rounded-lg flex items-center justify-center p-4 cursor-pointer border border-zinc-200 dark:border-zinc-700 group-hover:border-blue-500 transition-colors relative">
                                        ...
                                        {taggingStatus[item.id] && (
                                            <div className="absolute inset-0 bg-black/50 flex flex-col items-center justify-center rounded-lg">
                                                ...
                                                <p className="text-white text-xs mt-2">Etiketleniyor...</p>
                                            </div>
                                        )}
                                    </div>
