...
import type { DosyaSistemiOgesi, FileType } from '../types';
import { Klasor, Dosya } from '../types';
...
const FileIcons: Record<FileType, React.ReactNode> = {
  ...
  pdf: (
    ...
    >
      ...
      <path d='M10.3 21.3a1.4 1.4 0 0 1-1.3-2.1L10 16H8a2 2 0 0 1-2-2v-1a2 2 0 0 1 2-2h2.5a1.5 1.5 0 0 1 0 3H10l-1.5 2.5a1.4 1.4 0 0 1 .1 1.6 1.4 1.4 0 0 1-1.3.8Z' />
      <path d='M17 10.5a1.5 1.5 0 0 1 0 3h-1v-3a1.5 1.5 0 0 1 3 0v3h-1' />
...
const DokumanArsivi: React.FC = () => {
  ...
  const [taggingStatus, setTaggingStatus] = useState<Record<string, boolean>>(
    {}
  );
    ...
    if (file) {
      ...
      try {
        ...
        if (newFile.fileType === 'image') {
          setTaggingStatus(prev => ({ ...prev, [newFile.id]: true }));
          const tags = await huggingfaceService.getImageTags(file);

          (async () => {
            if (Array.isArray(tags) && tags.length > 0) {
              const updatedFileWithTags = await updateFile(newFile.id, {
                tags
              });
              setItems(prev =>
                prev.map(item =>
                  item.id === newFile.id ? updatedFileWithTags : item
                )
              );
            }
          })();
  ...
  const handleDeleteItem = async (item: DosyaSistemiOgesi) => {
    if (
      ...
      try {
        ...
        alert(`Öğe silinemedi: ${err.message}`);
      }
    }
  };
        ...
        <div className='p-4 border-b border-zinc-200 dark:border-zinc-700 flex flex-col md:flex-row items-center justify-between gap-4'>
          ...
          <div className='flex items-center gap-2'>
            <input
              ...
              className='px-3 py-2 border border-zinc-300 dark:border-zinc-600 rounded-lg text-sm w-48 bg-white dark:bg-zinc-700'
            />
            <button
              onClick={handleCreateFolder}
              className='bg-zinc-100 dark:bg-zinc-700 text-zinc-700 dark:text-zinc-200 px-3 py-2 rounded-lg text-sm font-semibold hover:bg-zinc-200 dark:hover:bg-zinc-600'
            >
              Yeni Klasör
            </button>
            <input
        ...
        <div className='flex-1 p-6 overflow-y-auto'>
          {isLoading ? (
              ...
              {filteredItems.map(item => (
                ...
                >
                  <div className='aspect-w-1 aspect-h-1 w-full bg-zinc-100 dark:bg-zinc-700/50 rounded-lg flex items-center justify-center p-4 cursor-pointer border border-zinc-200 dark:border-zinc-700 group-hover:border-blue-500 transition-colors relative'>
                    {item.type === 'folder' ? (
                      <span className='text-zinc-400 dark:text-zinc-500 w-16 h-16'>
                        {FolderIcon}
                      </span>
                      ...
                      <span className='text-zinc-400 dark:text-zinc-500 w-12 h-12'>
                        {FileIcons[item.fileType]}
                      </span>
                    )}
                    {taggingStatus[item.id] && (
                    ...
                    {item.type === 'file' && item.tags && (
                      <div className='flex flex-wrap gap-1 justify-center mt-1'>
                        {item.tags.map(tag => (
                          <span
                            key={tag}
                            className='text-xs bg-purple-100 text-purple-800 dark:bg-purple-900/50 dark:text-purple-300 font-medium px-1.5 py-0.5 rounded'
                          >
                            {tag}
                          </span>
                        ))}
            ...
            <div className='text-center py-20 text-zinc-500 dark:text-zinc-400'>
              <svg
                xmlns='http://www.w3.org/2000/svg'
                className='mx-auto h-16 w-16 text-zinc-400 dark:text-zinc-500'
                fill='none'
                viewBox='0 0 24 24'
                stroke='currentColor'
                strokeWidth='1'
              ...
              <p className='mt-1 text-sm text-zinc-500 dark:text-zinc-400'>
                Dosyaları buraya sürükleyip bırakabilir veya yukarıdan
                yükleyebilirsiniz.
              </p>
            </div>
          )}
