...
const ApiEntegrasyonu: React.FC = () => {
  ...
  const [modal, setModal] = useState<
    'newApiKey' | 'showApiKey' | 'webhook' | null
  >(null);
  const [newlyGeneratedKey, setNewlyGeneratedKey] = useState<string | null>(
    null
  );
  const [editingWebhook, setEditingWebhook] = useState<Partial<Webhook> | null>(
    null
  );
  ...
  const handleSaveApiKey = async (name: string) => {
    try {
      ...
      alert(`API anahtarı oluşturulurken bir hata oluştu: ${err.message}`);
    }
  };
  ...
  const handleRevokeKey = async (id: number) => {
    if (
      window.confirm(
        'Bu API anahtarını iptal etmek istediğinizden emin misiniz? Bu işlem geri alınamaz.'
      )
    ) {
      try {
        ...
        alert(`API anahtarı iptal edilirken bir hata oluştu: ${err.message}`);
      }
    }
  };
    ...
    try {
      if (webhookToSave.id) {
        ...
        const created = await createWebhook(
          webhookToSave as Omit<Webhook, 'id'>
        );
      ...
      alert(`Webhook kaydedilirken bir hata oluştu: ${err.message}`);
    }
  };
  ...
  const handleDeleteWebhook = async (id: number) => {
    if (window.confirm('Bu webhooku silmek istediğinizden emin misiniz?')) {
      try {
        ...
        alert(`Webhook silinirken bir hata oluştu: ${err.message}`);
      }
    }
  };
              ...
              >
                <div>
                  ...
                  <p className='text-xs text-slate-400'>
                    Oluşturulma:{' '}
                    {new Date(key.createdDate).toLocaleDateString('tr-TR')}
                  </p>
                </div>
                <div className='flex items-center gap-3'>
                  <span
                    className={`px-2 py-1 text-xs font-medium rounded-full ${key.status === 'Active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}
                  >
                    {key.status}
                  </span>
                  <button
                  ...
                  >
                    {key.status === 'Active' ? 'İptal Et' : 'İptal Edildi'}
                  </button>
                </div>
              </li>
            ))}
              ...
              >
                <div>
                  ...
                  <p className='text-sm text-slate-500'>
                    Tetikleyici Olay:{' '}
                    <span className='font-semibold'>{wh.event}</span>
                  </p>
                </div>
                <div className='flex items-center gap-3'>
                  <span
                    className={`px-2 py-1 text-xs font-medium rounded-full ${wh.status === 'Active' ? 'bg-green-100 text-green-800' : 'bg-slate-200 text-slate-800'}`}
                  >
                    {wh.status}
                  </span>
                  <button
...
const ApiKeyFormModal: React.FC<{
  onClose: () => void;
      ...
      <form onSubmit={handleSubmit} className='space-y-4'>
        <div>
          ...
          <p className='text-xs text-slate-500 mt-1'>
            Bu anahtarı ne için kullanacağınızı belirten bir ad girin.
          </p>
        </div>
...
const ShowKeyModal: React.FC<{ apiKey: string; onClose: () => void }> = ({
  apiKey,
  onClose
}) => {
    ...
    <Modal isOpen={true} onClose={onClose} title='API Anahtarınız Oluşturuldu'>
      <div className='space-y-4'>
        <div className='p-4 bg-yellow-50 border border-yellow-200 rounded-lg text-yellow-800 text-sm'>
          Lütfen bu anahtarı güvenli bir yere kaydedin. Bu pencereyi kapattıktan
          sonra anahtarın tamamını bir daha göremezsiniz.
        </div>
        <div className='p-3 bg-slate-100 rounded-md font-mono text-slate-700 flex justify-between items-center'>
          <span>{apiKey}</span>
          <button
            onClick={copyToClipboard}
            className='bg-slate-200 px-3 py-1 rounded-md text-sm font-sans font-semibold hover:bg-slate-300'
          >
...
const WebhookFormModal: React.FC<{
  ...
  return (
    <Modal
      isOpen={true}
      onClose={onClose}
      title={webhook.id ? "Webhook'u Düzenle" : 'Yeni Webhook Oluştur'}
    >
      <form onSubmit={handleSubmit} className='space-y-4'>
        <div>
          <label className='block text-sm font-medium text-slate-700'>
