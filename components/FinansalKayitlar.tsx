...
import type { FinansalKayit, Proje } from '../types';
import { FinansalIslemTuru, HesapKategorisi } from '../types';
...
const FinansalKayitlar: React.FC = () => {
  ...
  const [typeFilter, setTypeFilter] = useState<FinansalIslemTuru | 'all'>(
    'all'
  );
  const [categoryFilter, setCategoryFilter] = useState<HesapKategorisi | 'all'>(
    'all'
  );

  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingKayit, setEditingKayit] =
    useState<Partial<FinansalKayit> | null>(null);

  const fetchData = async () => {
    setIsLoading(true);
    setError('');
    try {
      const [kayitlarData, projelerData] = await Promise.all([
        getFinansalKayitlar(),
        getProjeler()
      ]);
    ...
    const filtered = kayitlar
      .filter(kayit => {
        const matchesSearch =
          ...
          (kayit.belgeNo &&
            kayit.belgeNo.toLowerCase().includes(searchTerm.toLowerCase()));
        const matchesType = typeFilter === 'all' || kayit.tur === typeFilter;
        const matchesCategory =
          categoryFilter === 'all' || kayit.kategori === categoryFilter;
  ...
  const handleSaveKayit = async (kayitToSave: Partial<FinansalKayit>) => {
    try {
      if (kayitToSave.id) {
        ...
        const newKayit = await createFinansalKayit(
          kayitToSave as Omit<FinansalKayit, 'id'>
        );
    ...
    <>
      <div className='space-y-6'>
        <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
          <StatCard
            title='Toplam Gelir'
            value={totalGelir.toLocaleString('tr-TR', {
              style: 'currency',
              currency: 'TRY'
            })}
          ...
          <StatCard
            title='Bakiye'
            value={bakiye.toLocaleString('tr-TR', {
              style: 'currency',
              currency: 'TRY'
            })}
          ...
          <div className='flex flex-col md:flex-row items-center justify-between space-y-4 md:space-y-0 mb-6'>
            ...
            <div className='flex items-center space-x-4'>
              <select
                ...
                onChange={e =>
                  setCategoryFilter(e.target.value as HesapKategorisi | 'all')
                }
              >
                <option value='all'>Tüm Kategoriler</option>
                {Object.values(HesapKategorisi).map(cat => (
              ...
              <select
                ...
                value={typeFilter}
                onChange={e =>
                  setTypeFilter(e.target.value as FinansalIslemTuru | 'all')
                }
              >
            ...
            <table className='w-full text-sm text-left text-slate-500'>
              ...
              <tbody className='divide-y divide-slate-200'>
                {filteredKayitlar.map(kayit => {
                  const ilgiliProje = kayit.projeId
                    ? projects.find(p => p.id === kayit.projeId)
                    : null;
                  return (
                    <tr key={kayit.id} className='hover:bg-slate-50'>
                        ...
                        <div className='flex items-center space-x-2'>
                          {kayit.belgeNo && (
                            <div className='text-xs text-slate-500'>
                              Belge No: {kayit.belgeNo}
                            </div>
                          )}
                          {ilgiliProje && (
                            <div className='text-xs text-purple-600 font-semibold bg-purple-100 px-1.5 py-0.5 rounded-full'>
                              {ilgiliProje.name}
                            </div>
                          )}
                        </div>
                      ...
                      <td
                        className={`px-6 py-4 font-semibold ${kayit.tur === FinansalIslemTuru.GELIR ? 'text-green-600' : 'text-red-600'}`}
                      >
                        {kayit.tur === FinansalIslemTuru.GELIR ? '+' : '-'}{' '}
                        {kayit.tutar.toLocaleString('tr-TR', {
                          style: 'currency',
                          currency: 'TRY'
                        })}
                      </td>
                      <td className='px-6 py-4 text-right'>
                        <div className='flex items-center justify-end space-x-4'>
                          <button
                            onClick={() => handleEditClick(kayit)}
                            className='text-yellow-600 hover:text-yellow-800 font-semibold'
                          >
                            Düzenle
                          </button>
                          <button
                            onClick={() => handleDeleteClick(kayit.id)}
...
export const FinansalKayitFormModal: React.FC<{
  ...
  return (
    <Modal
      isOpen={true}
      onClose={onClose}
      title={isNew ? 'Yeni Finansal Kayıt Ekle' : 'Finansal Kaydı Düzenle'}
    >
