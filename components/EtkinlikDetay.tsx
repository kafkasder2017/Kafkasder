...
import { Etkinlik, EtkinlikKatilimcisi, Person, Gonullu, Beceri, VolunteerSuggestion, EtkinlikStatus } from '../types';
import { getEtkinlikById, getGonulluler, getPeople, updateEtkinlik } from '../services/apiService';
import { suggestVolunteersForEvent, EnrichedGonullu } from '../services/geminiService';
...
const EtkinlikDetay: React.FC = () => {
    ...
    const handleGetAiSuggestions = async () => {
        ...
        try {
            const alreadyParticipatingIds = new Set(etkinlik.katilimcilar?.map(p => p.personId) || []);
            const availableVolunteers = gonulluler.filter(g => !alreadyParticipatingIds.has(g.personId));
            
            const suggestions = await suggestVolunteersForEvent(etkinlik, availableVolunteers);
    ...
    const handleAddParticipant = async (personId: number) => {
        ...
        const isAlreadyParticipant = etkinlik.katilimcilar?.some(p => p.personId === personId);
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-zinc-800 p-6 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-700">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <button onClick={handleGetAiSuggestions} className="bg-purple-600 text-white px-4 py-2 rounded-lg font-semibold hover:bg-purple-700 transition-colors flex items-center space-x-2 text-sm">
...
const AiSuggestionsModal: React.FC<{
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="AI Destekli Gönüllü Önerileri">
            <div className="min-h-[20rem]">
                {isLoading ? (
                    <div className="flex flex-col items-center justify-center h-full">
                        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-purple-600"></div>
                        <p className="mt-4 text-zinc-500">Gönüllüler etkinlik detaylarına göre analiz ediliyor...</p>
                    </div>
                ) : error ? (
                    ...
                    <ul className="space-y-3">
                        {suggestions.map(s => (
                            <li key={s.personId} className="p-3 bg-zinc-50 dark:bg-zinc-700/50 rounded-lg flex items-center justify-between">
                                <div>
                                    <p className="font-bold text-zinc-800 dark:text-zinc-200">{s.volunteerName}</p>
                                    <p className="text-sm text-zinc-600 dark:text-zinc-300 italic mt-1">"{s.reasoning}"</p>
                                    <div className="flex flex-wrap gap-1 mt-2">
                                        {s.relevantSkills.map(skill => <span key={skill} className="text-xs bg-blue-100 text-blue-800 dark:bg-blue-900/50 dark:text-blue-300 font-medium px-1.5 py-0.5 rounded">{skill}</span>)}
                                    </div>
                                </div>
                                <button onClick={() => onAddParticipant(s.personId)} className="bg-green-100 text-green-700 dark:bg-green-900/50 dark:text-green-300 px-3 py-1 rounded-full text-sm font-bold hover:bg-green-200 dark:hover:bg-green-900 ml-4">
                                    Ekle
                                </button>
                            </li>
                        ))}
                    ...
                    <p className="text-center text-zinc-500 py-10">Uygun gönüllü önerisi bulunamadı.</p>
                )}
            </div>
             <div className="pt-4 mt-4 border-t border-zinc-200 dark:border-zinc-700 flex justify-end">
                <button type="button" onClick={onClose} className="bg-zinc-200 dark:bg-zinc-700 text-zinc-800 dark:text-zinc-200 px-4 py-2 rounded-lg font-semibold hover:bg-zinc-300 dark:hover:bg-zinc-600">Kapat</button>
            </div>
        </Modal>
    );
};
