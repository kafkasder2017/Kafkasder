...
import type { VefaDestek, VefaNotu, Odeme } from '../types';
...
const VefaDestekDetay: React.FC = () => {
  ...
  const supportHistory = useMemo(() => {
    ...
    return payments.filter(
      o => o.kisi === vefa.adiSoyadi && o.odemeTuru === OdemeTuru.VEFA_DESTEGI
    );
  }, [vefa, payments]);
  ...
  return (
    <div className='space-y-6'>
      <div className='bg-white p-6 rounded-lg shadow-sm flex items-center space-x-6'>
        <img
          src={`https://i.pravatar.cc/100?u=${vefa.adiSoyadi}`}
          alt={vefa.adiSoyadi}
        ...
        <div>
          ...
          <p className='text-slate-500 mt-1'>
            Sorumlu Gönüllü: {vefa.sorumluGonullu}
          </p>
        </div>
      </div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6'>
        <StatCard
          label='Yaş'
          value={calculateAge(vefa.dogumTarihi)}
          icon={
            <svg
  ...
  if (!history.length)
    return (
      <p className='text-sm text-slate-500 text-center py-4'>
        Bu kişiye ait destek ödemesi kaydı bulunamadı.
      </p>
    );
  return (
...
const VisitNotes: React.FC<{ history: VefaNotu[]; onAddNote: () => void }> = ({
  history,
  onAddNote
}) => {
  return (
...
const VefaNotuModal: React.FC<{
  onClose: () => void;
  onSave: (note: Omit<VefaNotu, 'id'>) => void;
}> = ({ onClose, onSave }) => {
  ...
  return (
    <Modal
      isOpen={true}
      onClose={onClose}
      title='Yeni Ziyaret/Destek Notu Ekle'
    >
      <form onSubmit={handleSubmit} className='space-y-4'>
        ...
        <div>
          ...
          <textarea
            ...
            rows={4}
            className='mt-1 block w-full p-2 border border-slate-300 rounded-md'
            placeholder='Ziyaret, arama, sağlık durumu vb. bilgiler'
            required
          />
        </div>
