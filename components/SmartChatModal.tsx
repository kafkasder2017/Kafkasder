...
const SmartChatModal = <T extends {}>({
  isOpen,
  onClose,
  fullDataset,
  onResults,
  entityName,
  exampleQuery
}: SmartChatModalProps<T>) => {
  const [history, setHistory] = useState<ChatMessage[]>([]);
  const [currentFilteredData, setCurrentFilteredData] = useState<T[] | null>(
    null
  );
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={`Akıllı ${entityName.charAt(0).toUpperCase() + entityName.slice(1)} Filtreleme Sohbeti`}
    >
      <div className='flex flex-col h-[60vh] bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-700 rounded-lg'>
        <div className='flex-1 p-4 overflow-y-auto space-y-4'>
          {history.map((msg, index) => (
            ...
            >
              {msg.role === 'model' && (
                <div className='w-8 h-8 bg-zinc-700 text-white rounded-full flex items-center justify-center flex-shrink-0 text-sm font-bold dark:bg-zinc-600'>
                  AI
                </div>
              )}
              <div
                className={`max-w-md p-3 rounded-2xl shadow-sm ${msg.role === 'user' ? 'bg-blue-600 text-white rounded-br-none' : 'bg-white dark:bg-zinc-800 text-zinc-800 dark:text-zinc-200 rounded-bl-none border dark:border-zinc-700'}`}
              >
