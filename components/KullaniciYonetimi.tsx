...
const getStatusClass = (status: KullaniciDurum) => {
    return status === KullaniciDurum.AKTIF ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800';
};

const getRoleClass = (role: KullaniciRol) => {
    switch (role) {
        case KullaniciRol.YONETICI: return 'bg-indigo-100 text-indigo-800';
        case KullaniciRol.EDITOR: return 'bg-blue-100 text-blue-800';
        case KullaniciRol.MUHASEBE: return 'bg-amber-100 text-amber-800';
        case KullaniciRol.GONULLU: return 'bg-purple-100 text-purple-800';
...
const KullaniciYonetimi: React.FC = () => {
    ...
    const [statusFilter, setStatusFilter] = useState<KullaniciDurum | 'all'>('all');
    ...
    const filteredKullanicilar = useMemo(() => {
        return kullanicilar.filter(kullanici => {
            ...
            const matchesStatus = statusFilter === 'all' || kullanici.durum === statusFilter;
    ...
    const handleSaveKullanici = async (kullaniciToSave: Partial<Kullanici>) => {
        try {
            if (kullaniciToSave.id) { // Editing
                const updatedUser = await updateUser(kullaniciToSave.id, kullaniciToSave);
                setKullanicilar(kullanicilar.map(k => k.id === kullaniciToSave.id ? updatedUser : k));
            } else { // Adding new
    ...
    const toggleUserStatus = async (kullanici: Kullanici) => {
        ...
        try {
            ...
            setKullanicilar(kullanicilar.map(k => (k.id === kullanici.id ? updatedUser : k)));
        } catch (err) {
                ...
                <div className="flex flex-col md:flex-row items-center justify-between space-y-4 md:space-y-0 mb-6">
                    ...
                    <div className="flex items-center space-x-4">
                         <select 
                            ...
                            value={roleFilter}
                            onChange={(e) => setRoleFilter(e.target.value as KullaniciRol | 'all')}
                         >
                            <option value="all">Tüm Roller</option>
                            {Object.values(KullaniciRol).map(rol => <option key={rol} value={rol}>{rol}</option>)}
                        </select>
                        <select 
                            className="px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
                            value={statusFilter}
                            onChange={(e) => setStatusFilter(e.target.value as KullaniciDurum | 'all')}
                        >
                    ...
                    <table className="w-full text-sm text-left text-slate-500">
                        ...
                        <tbody>
                            {filteredKullanicilar.map((kullanici) => (
                                <tr key={kullanici.id} className="bg-white border-b hover:bg-slate-50">
                                    <td className="px-6 py-4 font-medium text-slate-900 whitespace-nowrap">{kullanici.kullaniciAdi}</td>
                                    <td className="px-6 py-4">{kullanici.email}</td>
                                    <td className="px-6 py-4">
                                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${getRoleClass(kullanici.rol)}`}>
                                            {kullanici.rol}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4">{kullanici.sonGiris ? new Date(kullanici.sonGiris).toLocaleString('tr-TR') : 'Hiç'}</td>
                                    <td className="px-6 py-4">
                                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusClass(kullanici.durum)}`}>
                                            {kullanici.durum}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex items-center justify-end space-x-3">
                                            <button onClick={() => handleEditClick(kullanici)} className="text-yellow-600 hover:text-yellow-800 font-medium">Düzenle</button>
                                            <button onClick={() => toggleUserStatus(kullanici)} className={`font-medium ${kullanici.durum === KullaniciDurum.AKTIF ? 'text-red-600 hover:text-red-800' : 'text-green-600 hover:text-green-800'}`}>
                                                {kullanici.durum === KullaniciDurum.AKTIF ? 'Pasif Yap' : 'Aktif Yap'}
                                            </button>
                                            <button onClick={() => handleDeleteClick(kullanici.id)} className="text-red-600 hover:text-red-800 font-medium">Sil</button>
                                        </div>
                                    </td>
                                </tr>
                            ))}
...
const KullaniciFormModal: React.FC<{ kullanici: Partial<Kullanici>, onClose: () => void, onSave: (kullanici: Partial<Kullanici>) => void }> = ({ kullanici, onClose, onSave }) => {
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="kullaniciAdi" value={formData.kullaniciAdi || ''} onChange={handleChange} className="mt-1 block w-full border border-slate-300 rounded-md shadow-sm p-2" required />
                    </div>
                     <div>
                    ...
                    <div>
                        ...
                        <select name="rol" value={formData.rol || ''} onChange={handleChange} className="mt-1 block w-full border border-slate-300 rounded-md shadow-sm p-2" required>
                     ...
                     {isNewUser && (
                        <div>
                            ...
                            <input type="password" name="password" placeholder="Yeni şifre belirle" className="mt-1 block w-full border border-slate-300 rounded-md shadow-sm p-2" required />
                        </div>
                     )}
                     {
                        !isNewUser && (
                             <div>
                                ...
                                <select name="durum" value={formData.durum || ''} onChange={handleChange} className="mt-1 block w-full border border-slate-300 rounded-md shadow-sm p-2" required>
                                    <option value="" disabled>Seçiniz...</option>
                                    {Object.values(KullaniciDurum).map(durum => <option key={durum} value={durum}>{durum}</option>)}
                                </select>
                            </div>
                        )
                     }
