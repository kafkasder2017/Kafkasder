...
import { validateEmail, sanitizeInput, preventXSS } from '../utils/validation';
...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      await onLogin(preventXSS(sanitizedEmail), preventXSS(sanitizedPassword));
    } catch (err: any) {
    ...
    <div className='flex items-center justify-center min-h-screen bg-zinc-50 dark:bg-zinc-900'>
      <div className='w-full max-w-md p-8 space-y-8 bg-white dark:bg-zinc-800 rounded-2xl shadow-lg'>
        <div className='text-center'>
          <h1 className='text-3xl font-bold text-zinc-800 dark:text-white tracking-wider'>
            KAFKASDER
          </h1>
