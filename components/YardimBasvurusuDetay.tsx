...
import { YardimBasvurusu, BasvuruStatus, BasvuruOncelik, DokumanTipi, PersonDocument, Person, Yorum, LogEntityType, Sentiment } from '../types';
...
import { summarizeUploadedDocument, analyzeCommentSentiment } from '../services/geminiService';
        ...
        <div className="group relative flex flex-col h-full bg-white dark:bg-zinc-800 rounded-lg border border-zinc-200 dark:border-zinc-700 transition-shadow hover:shadow-md">
            ...
            {doc.aiSummary && (
                 <div className="mt-auto p-2 border-t border-zinc-200 dark:border-zinc-700 bg-purple-50 dark:bg-purple-900/20">
                    <p className="text-xs text-purple-800 dark:text-purple-300 italic">{doc.aiSummary}</p>
                </div>
            )}
            <button
...
const YorumItem: React.FC<{ yorum: Yorum }> = ({ yorum }) => {
    const formattedContent = {
        __html: yorum.icerik.replace(/(@[\w\d_]+)/g, '<strong class="text-blue-600 font-semibold">$1</strong>')
    };
                ...
                return (
                    <div className="group relative">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-red-500" viewBox="0 0 20 20" fill="currentColor">
                            <path fillRule="evenodd" d="M8.257 3.099c.636-1.214 2.522-1.214 3.158 0l5.42 10.372c.636 1.214-.364 2.729-1.579 2.729H4.416c-1.215 0-2.215-1.515-1.579-2.729L8.257 3.099zM10 13a1 1 0 11-2 0 1 1 0 012 0zm-1-3a1 1 0 00-1 1v2a1 1 0 102 0v-2a1 1 0 00-1-1z" clipRule="evenodd" />
                        </svg>
                 ...
                 return (
                    <div className="group relative">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-amber-500" viewBox="0 0 20 20" fill="currentColor">
                            <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM7 8a1 1 0 11-2 0 1 1 0 012 0zm5 0a1 1 0 11-2 0 1 1 0 012 0zm-2.646 5.646a.5.5 0 01.708 0l.058.057 2 2a.5.5 0 01-.707.708L10 14.707l-1.707 1.707a.5.5 0 01-.708-.707l2-2 .058-.057z" clipRule="evenodd" />
                        </svg>
                         <div className="absolute bottom-full mb-2 w-48 bg-zinc-800 text-white text-xs rounded py-1 px-2 text-center opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
    ...
    return (
        <li className="flex items-start space-x-3">
            ...
            <div className="flex-1 bg-zinc-100 dark:bg-zinc-700/50 p-3 rounded-lg">
                ...
                <p className="text-sm text-zinc-700 dark:text-zinc-300 mt-1" dangerouslySetInnerHTML={formattedContent} />
            </div>
        </li>
    );
};
...
const YardimBasvurusuDetay: React.FC = () => {
    ...
    const handleAddComment = async (commentText: string) => {
        ...
        try {
            ...
            setBasvuru(prev => prev ? { ...prev, yorumlar: updatedYorumlar } : null);
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            setUploadError("Dosya yüklenirken veya özetlenirken bir hata oluştu: " + err.message);
            console.error(err);
        } finally {
                            ...
                            {(basvuru.dosyalar || []).map(doc => <FilePreviewCard key={doc.id} doc={doc} onDelete={handleFileDelete} />)}
                             {isUploading && (
                                <div className="aspect-w-1 aspect-h-1 w-full bg-zinc-100 dark:bg-zinc-800/50 rounded-lg flex flex-col items-center justify-center p-2 border-2 border-dashed border-zinc-300 dark:border-zinc-600">
                                    ...
                                    <p className="text-xs mt-2 text-zinc-500">Yükleniyor ve özetleniyor...</p>
                                </div>
                            )}
                        </div>
