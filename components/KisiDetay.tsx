...
const DetailItem: React.FC<{
  ...
  <div className={className}>
    ...
    <div className='mt-1 text-base text-zinc-800 dark:text-zinc-200'>
      {children || value || '-'}
    </div>
  </div>
);
...
const FormInput: React.FC<{
  ...
  <div className='flex flex-col'>
    ...
    <input
      ...
      required={required}
      disabled={disabled}
      className='p-2 border border-zinc-300 dark:border-zinc-600 rounded-md bg-white dark:bg-zinc-700 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 text-sm disabled:bg-zinc-100 dark:disabled:bg-zinc-700/50 disabled:cursor-not-allowed'
    />
  </div>
);
...
const FormSelect: React.FC<{
  ...
  <div className='flex flex-col'>
    ...
    <select
      ...
      required={required}
      disabled={disabled}
      className='p-2 border border-zinc-300 dark:border-zinc-600 rounded-md bg-white dark:bg-zinc-700 text-sm appearance-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 disabled:bg-zinc-100 dark:disabled:bg-zinc-700/50 disabled:cursor-not-allowed'
    >
...
const TabButton: React.FC<{
  ...
  <button
    onClick={onClick}
    className={`px-4 py-2 text-sm font-semibold rounded-md transition-colors ${active ? 'bg-blue-600 text-white' : 'text-zinc-600 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-700'}`}
  >
    {children}
  </button>
);
...
// --- MODAL COMPONENTS --- //
const BankaHesaplariModal: React.FC<{
  person: Person;
  onClose: () => void;
  ...
  const handleDelete = (id: string) =>
    setHesaplar(hesaplar.filter(h => h.id !== id));
      ...
      <div className='space-y-4'>
        ...
        <div className='flex gap-2 border-t pt-4'>
          <input
            ...
            onChange={e =>
              setNewHesap(p => ({ ...p, hesapAdi: e.target.value }))
            }
          ...
          <button
            onClick={handleAdd}
            className='bg-blue-600 text-white px-4 rounded-md font-semibold hover:bg-blue-700'
          >
            Ekle
          </button>
        </div>
...
const DokumanlarModal: React.FC<{
  person: Person;
  onClose: () => void;
  onSave: (data: Partial<Person>) => void;
}> = ({ person, onClose, onSave }) => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.success('Doküman yüklendi (AI özetleme devre dışı)');
    } catch (error) {
      console.error(error);
  ...
  const handleDelete = (id: string) =>
    setDokumanlar(dokumanlar.filter(d => d.id !== id));
      ...
      <div className='space-y-4'>
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-h-80 overflow-y-auto p-1'>
          {dokumanlar.length > 0 ? (
            dokumanlar.map(doc => (
              ...
              >
                <p
                  className='font-semibold text-sm truncate text-zinc-800 dark:text-zinc-200'
                  title={doc.ad}
                >
                  {doc.ad}
                </p>
                ...
                {doc.aiSummary && (
                  <p className='text-xs italic text-purple-600 dark:text-purple-400 mt-2'>
                    "{doc.aiSummary}"
                  </p>
                )}
                <button
        ...
        <div className='border-t pt-4'>
          ...
          >
            {isUploading
              ? 'Yükleniyor...'
              : 'Yeni Doküman Yükle (AI Özetleme Devre Dışı)'}
          </button>
        </div>
        <div className='flex justify-end gap-2 pt-4'>
          <button
...
const BagliKisilerModal: React.FC<{
  person: Person;
  allPeople: Person[];
  onClose: () => void;
  onSave: (data: Partial<Person>) => void;
  ...
  const availablePeople = allPeople.filter(
    p => p.id !== person.id && !dependents.some(d => d.personId === p.id)
  );
  ...
  const handleAdd = () => {
    if (newDependent.personId && newDependent.relationship) {
      setDependents([
        ...dependents,
        {
          personId: Number(newDependent.personId),
          relationship: newDependent.relationship as YakinlikTuru
        }
      ]);
  ...
  const handleDelete = (personId: number) =>
    setDependents(dependents.filter(d => d.personId !== personId));
          ...
          {dependents.length > 0 ? (
            dependents.map(dep => (
              ...
              >
                <div>
                  <p className='font-semibold text-zinc-800 dark:text-zinc-200'>
                    {peopleMap.get(dep.personId) || 'Bilinmeyen Kişi'}
                  </p>
        ...
        <div className='flex gap-2 border-t pt-4'>
          <div className='flex-1'>
            <SearchableSelect<Person>
              ...
              value={newDependent.personId}
              onChange={val => setNewDependent(p => ({ ...p, personId: val }))}
          ...
          <select
            ...
            onChange={e =>
              setNewDependent(p => ({
                ...p,
                relationship: e.target.value as YakinlikTuru
              }))
            }
...
const KisiDetay: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError('');
    try {
      const [personData, allPeopleData] = await Promise.all([
        getPersonById(parseInt(kisiId, 10)),
        getPeople()
      ]);
  ...
  if (!person || !formData)
    return <div className='text-center py-10'>Kişi bulunamadı.</div>;

  const fullNameForUrl = encodeURIComponent(`${person.ad} ${person.soyad}`);
  const linkedRecordsConfig = [
    {
      label: 'Banka Hesapları',
      action: () => setActiveModal('banka'),
      count: person.bankaHesaplari?.length
    },
    {
      label: 'Dokümanlar',
      action: () => setActiveModal('dokuman'),
      count: person.dokumanlar?.length
    },
    { label: 'Fotoğraflar', disabled: true, count: person.fotograflar?.length },
    { label: 'Bağlı Yetimler', disabled: true },
    {
      label: 'Baktığı Kişiler',
      action: () => setActiveModal('bagli-kisiler'),
      count: person.dependents?.length
    },
    ...
    { label: 'Yardım Talepleri', to: `/yardimlar?kisiAdi=${fullNameForUrl}` },
    {
      ...
      to: `/yardim-yonetimi/tum-yardimlar?kisiAdi=${fullNameForUrl}`
    },
  ...
  const renderContent = () => {
    const View = (
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-8'>
        <DetailItem
          label='Ad Soyad'
          value={`${formData.ad} ${formData.soyad}`}
        />
        <DetailItem
          label='Durum'
          children={
            <span
              className={`px-2.5 py-1 text-xs font-semibold rounded-full ${getStatusClass(formData.durum)}`}
            >
              {formData.durum}
            </span>
          }
        ...
        <DetailItem
          label='Doğum Tarihi'
          value={new Date(formData.dogumTarihi).toLocaleDateString('tr-TR')}
        />
        ...
        <DetailItem
          label='Adres'
          value={`${formData.adres}, ${formData.mahalle}, ${formData.yerlesim}/${formData.sehir}`}
    ...
    const Edit = (
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
        ...
        <FormSelect
          ...
          onChange={handleInputChange}
          options={Object.values(PersonStatus).map(s => ({
            value: s,
            label: s
          }))}
          required
        ...
        <FormSelect
          ...
          onChange={handleInputChange}
          options={Object.values(MedeniDurum).map(s => ({
            value: s,
            label: s
          }))}
        />
  ...
  return (
    <div className='space-y-4'>
      <div className='bg-white dark:bg-zinc-800 p-4 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-700 flex items-center justify-between'>
        <h2 className='text-xl font-bold text-zinc-900 dark:text-zinc-100'>
          {person.ad} {person.soyad} (ID: #{person.id})
        </h2>
        <div>
          {isEditMode ? (
            <div className='flex gap-2'>
              <button
                ...
                className='px-4 py-2 text-sm font-semibold text-zinc-700 bg-white border border-zinc-300 rounded-md hover:bg-zinc-50'
              >
                İptal
              </button>
              <button
                ...
                disabled={isSaving}
                className='px-4 py-2 text-sm font-semibold text-white bg-blue-600 rounded-md hover:bg-blue-700 disabled:bg-blue-400'
              >
