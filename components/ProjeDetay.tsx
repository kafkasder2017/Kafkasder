...
import type { Proje, Gorev, Person } from '../types';
import { GorevStatus, GorevOncelik } from '../types';
...
const getGorevStatusClass = (status: GorevStatus) => {
  switch (status) {
    case GorevStatus.YAPILACAK:
      return 'bg-slate-200 text-slate-800';
    case GorevStatus.YAPILIYOR:
      return 'bg-yellow-100 text-yellow-800';
    case GorevStatus.TAMAMLANDI:
...
const ProjeDetay: React.FC = () => {
  ...
  const handleSaveGorev = (gorevToSave: Gorev) => {
    ...
    if (gorevToSave.id) {
      // Editing
      updatedGorevler = (project.gorevler || []).map(g =>
        g.id === gorevToSave.id ? gorevToSave : g
      );
    } else {
      // Adding
  ...
  const handleGorevStatusChange = (id: number, newStatus: GorevStatus) => {
    if (!project) return;
    const updatedGorevler = (project.gorevler || []).map(g =>
      g.id === id ? { ...g, durum: newStatus } : g
    );
    onUpdateProject({ ...project, gorevler: updatedGorevler });
  };
  ...
  if (!project) {
    return (
      <div className='text-center py-20 text-slate-500'>Proje bulunamadı.</div>
    );
  }
  ...
  return (
    <div className='space-y-6'>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6'>
        ...
        <StatCard
          label='Proje Takvimi'
          value={`${new Date(project.startDate).toLocaleDateString('tr-TR')} - ${new Date(project.endDate).toLocaleDateString('tr-TR')}`}
          icon={
            <svg
              xmlns='http://www.w3.org/2000/svg'
              width='24'
        ...
        <StatCard
          label='Bütçe Durumu'
          value={`${project.spent.toLocaleString()} / ${project.budget.toLocaleString()} TL`}
          icon={
            <svg
              xmlns='http://www.w3.org/2000/svg'
              width='24'
        ...
        <StatCard
          ...
          icon={
            <svg
              ...
              strokeLinejoin='round'
            >
              <path d='M21.2 15c.7-1.2 1-2.5.7-3.9-.6-2.4-3-4-5.4-4.4'></path>
              <path d='M4.8 9A7.3 7.3 0 0 0 2 15c.7 1.2 1 2.5.7 3.9.6 2.4 3 4 5.4 4.4'></path>
              <path d='M12 21a7.3 7.3 0 0 1-5.2-12.2'></path>
              <path d='M12 3a7.3 7.3 0 0 0 5.2 12.2'></path>
        ...
        <div className='flex flex-col md:flex-row items-center justify-between space-y-4 md:space-y-0 mb-6'>
          <div className='flex items-center space-x-4'>
            ...
            <select
              ...
              value={statusFilter}
              onChange={e =>
                setStatusFilter(e.target.value as GorevStatus | 'all')
              }
            >
              <option value='all'>Tüm Durumlar</option>
              {Object.values(GorevStatus).map(s => (
                ...
                return (
                  ...
                  >
                    ...
                    <td className='px-6 py-4'>
                      {sorumlu ? `${sorumlu.ad} ${sorumlu.soyad}` : 'Atanmamış'}
                    </td>
                    <td className='px-6 py-4'>
                      {new Date(gorev.sonTarih).toLocaleDateString('tr-TR')}
                    </td>
                    <td className='px-6 py-4'>
                      <div className='flex items-center'>
                        <span
                          className={`w-3 h-3 rounded-full mr-2 ${getGorevOncelikClass(gorev.oncelik)}`}
                        ></span>
                        {gorev.oncelik}
                      </div>
                    </td>
                    <td className='px-6 py-4'>
                      <select
                        value={gorev.durum}
                        onChange={e =>
                          handleGorevStatusChange(
                            gorev.id,
                            e.target.value as GorevStatus
                          )
                        }
                    ...
                    <td className='px-6 py-4 text-right'>
                      <div className='flex items-center justify-end space-x-3'>
                        ...
                        <button
                          onClick={() => handleDeleteGorev(gorev.id)}
...
const GorevFormModal: React.FC<{
  gorev: Partial<Gorev>;
  people: Person[];
  onClose: () => void;
  onSave: (gorev: Gorev) => void;
}> = ({ gorev, people, onClose, onSave }) => {
  ...
  return (
    <Modal
      isOpen={true}
      onClose={onClose}
      title={gorev.id ? 'Görevi Düzenle' : 'Yeni Görev Ekle'}
    >
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
          ...
          <div>
            ...
            >
              ...
              {Object.values(GorevOncelik).map(s => (
                <option key={s} value={s}>
                  {s}
                </option>
              ))}
