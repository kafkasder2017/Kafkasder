...
import type { GonderilenMesaj } from '../types';
import { GonderimTuru, HedefKitle, MembershipType } from '../types';


const TopluIletisim: React.FC = () => {
  const [gonderimTuru, setGonderimTuru] = useState<GonderimTuru>(
    GonderimTuru.SMS
  );
  const [hedefKitle, setHedefKitle] = useState<HedefKitle>(
    HedefKitle.TUM_KISILER
  );
  const [baslik, setBaslik] = useState('');
  const [icerik, setIcerik] = useState('');
  const [gonderilenler, setGonderilenler] = useState<GonderilenMesaj[]>([]);

  // const [isGenerating, setIsGenerating] = useState(false); // AI generation state removed
      ...
      try {
        ...
        switch (hedefKitle) {
          ...
          case HedefKitle.TUM_UYELER:
            ...
            count = allPeople.filter(
              p =>
                p.membershipType && p.membershipType !== MembershipType.GONULLU
            ).length;
            break;
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    if (!icerik || recipientCount === 0) {
      setError(
        'Mesaj içeriği boş olamaz ve hedef kitle en az 1 kişi olmalıdır.'
      );
      return;
    }
    setIsSending(true);
    ...
    try {
      ...
      setStatus(
        `Mesajınız ${recipientCount} kişiye başarıyla gönderildi (simülasyon).`
      );
      setBaslik('');
      setIcerik('');
    } catch (err: any) {
        ...
        <form onSubmit={handleSendMessage} className='space-y-4'>
          <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
            <div>
              ...
              <select
                value={gonderimTuru}
                onChange={e => setGonderimTuru(e.target.value as GonderimTuru)}
            ...
            <div>
              <label className='block text-sm font-medium text-slate-700'>
                Hedef Kitle{' '}
                {isCalculating ? (
                  <span className='text-purple-600'>(Hesaplanıyor...)</span>
                ) : (
              ...
              <select
                ...
                className='mt-1 block w-full p-2 border border-slate-300 rounded-lg bg-white'
              >
                {Object.values(HedefKitle).map(k => (
          ...
          <div>
            ...
            <div>
              <input
                ...
                placeholder='örn: Bayram Tebriği, Etkinlik Duyurusu'
          ...
          <div>
            ...
            <textarea
              ...
              rows={gonderimTuru === GonderimTuru.SMS ? 4 : 8}
              className='mt-1 block w-full p-2 border border-slate-300 rounded-lg'
              required
            ></textarea>
            <p className='text-xs text-slate-500 mt-1'>
              {gonderimTuru === GonderimTuru.SMS
                ? `${icerik.length} karakter. (Bir SMS yaklaşık 160 karakterdir)`
          ...
          <div className='text-right pt-2'>
            <button
              type='submit'
              disabled={
                isSending || isCalculating || recipientCount === 0 || !icerik
              }
              className='bg-blue-600 text-white px-6 py-2 rounded-lg font-semibold hover:bg-blue-700 disabled:bg-blue-300 disabled:cursor-not-allowed'
            >
        ...
        <div className='space-y-3 max-h-[70vh] overflow-y-auto'>
          {gonderilenler.length > 0 ? (
            gonderilenler.map(msg => (
              <div key={msg.id} className='p-3 bg-slate-50 rounded-md'>
                ...
                <div className='flex justify-between items-center mt-2 text-xs text-slate-500'>
                  <span>
                    {msg.hedefKitle} ({msg.kisiSayisi} kişi)
                  </span>
                  <span>
                    {new Date(msg.gonderimTarihi).toLocaleDateString('tr-TR')}
                  </span>
                </div>
              </div>
            ))
            ...
            <p className='text-sm text-center text-slate-500 py-8'>
              Henüz gönderilmiş bir mesaj yok.
            </p>
          )}
        </div>
      </div>
