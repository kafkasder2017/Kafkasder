...
import { GonderimTuru } from '../types';
...
const MesajRaporlari: React.FC = () => {
  ...
  const { stats, byTypeData, byAudienceData, monthlyTrendData } =
    useMemo(() => {
      const totalMessages = messages.length;
      const totalRecipients = messages.reduce(
        (sum, msg) => sum + msg.kisiSayisi,
        0
      );
      const smsCount = messages.filter(
        m => m.gonderimTuru === GonderimTuru.SMS
      ).length;
      const emailCount = messages.filter(
        m => m.gonderimTuru === GonderimTuru.EPOSTA
      ).length;
      ...
      const byAudience = messages.reduce(
        (acc, m) => {
      ...
      const byAudienceData = Object.entries(byAudience).map(
        ([name, value]) => ({ name, value })
      );

      const monthly = messages.reduce(
        (acc, m) => {
      ...
      const monthlyTrendData = Object.entries(monthly)
