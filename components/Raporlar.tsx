...
import { getPeople, getYardimBasvurulari, getFinansalKayitlar } from '../services/apiService';
import { generateAnalyticsSummary } from '../services/geminiService';
import { Person, YardimBasvurusu, FinansalKayit, FinansalIslemTuru, AnalyticsSummary } from '../types';
...
const AiSummaryCard: React.FC<{ onGenerate: () => void; summary: AnalyticsSummary | null; isLoading: boolean; error: string }> = ({ onGenerate, summary, isLoading, error }) => (
    <div className="bg-white dark:bg-zinc-800 p-6 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-700 lg:col-span-2">
        <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={onGenerate} disabled={isLoading} className="bg-purple-600 text-white px-4 py-2 rounded-lg font-semibold hover:bg-purple-700 transition-colors flex items-center space-x-2 text-sm disabled:bg-purple-400 dark:disabled:bg-purple-800 disabled:cursor-not-allowed">
                {isLoading ? (
        ...
        {error && <div className="p-3 bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-300 rounded-md text-sm">{error}</div>}
        {summary ? (
            <div className="space-y-4 text-sm">
                <p className="text-zinc-700 dark:text-zinc-300 italic border-l-4 border-purple-400 pl-4 py-2 bg-purple-50 dark:bg-purple-900/20 rounded-r-md">{summary.summary}</p>
            ...
            <div className="text-center text-zinc-500 dark:text-zinc-400 py-10">Raporun genel bir özetini ve içgörüleri almak için "Özet Oluştur" butonuna tıklayın.</div>
        )}
    </div>
);
