...
import { FinansalIslemTuru } from '../types';
...
const AiSummaryCard: React.FC<{
  ...
  isLoading: boolean;
  error: string;
}> = ({ onGenerate, summary, isLoading, error }) => (
  <div className='bg-white dark:bg-zinc-800 p-6 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-700 lg:col-span-2'>
    <div className='flex justify-between items-center mb-4'>
      <h3 className='text-lg font-semibold text-zinc-800 dark:text-zinc-200 flex items-center gap-2'>
        <span className='w-6 h-6 text-purple-500'>{ICONS.LIGHTBULB}</span>
        AI Destekli Rapor Özeti
      </h3>
      <button
        onClick={onGenerate}
        disabled={isLoading}
        className='bg-purple-600 text-white px-4 py-2 rounded-lg font-semibold hover:bg-purple-700 transition-colors flex items-center space-x-2 text-sm disabled:bg-purple-400 dark:disabled:bg-purple-800 disabled:cursor-not-allowed'
      >
        {isLoading ? (
    ...
    {summary ? (
      <div className='space-y-4 text-sm'>
        <p className='text-zinc-700 dark:text-zinc-300 italic border-l-4 border-purple-400 pl-4 py-2 bg-purple-50 dark:bg-purple-900/20 rounded-r-md'>
          {summary.summary}
        </p>
        <div className='grid grid-cols-1 md:grid-cols-3 gap-4 pt-4'>
          <div>
            ...
            <ul className='list-disc list-inside mt-1 space-y-1 text-zinc-600 dark:text-zinc-400'>
              {summary.positiveTrends.map((item, i) => (
                <li key={i}>{item}</li>
              ))}
            </ul>
          </div>
          <div>
            ...
            <ul className='list-disc list-inside mt-1 space-y-1 text-zinc-600 dark:text-zinc-400'>
              {summary.areasForAttention.map((item, i) => (
                <li key={i}>{item}</li>
              ))}
            </ul>
          </div>
          <div>
            ...
            <ul className='list-disc list-inside mt-1 space-y-1 text-zinc-600 dark:text-zinc-400'>
              {summary.actionableInsights.map((item, i) => (
                <li key={i}>{item}</li>
              ))}
            </ul>
          </div>
      ...
      !error && (
        <div className='text-center text-zinc-500 dark:text-zinc-400 py-10'>
          Raporun genel bir özetini ve içgörüleri almak için "Özet Oluştur"
          butonuna tıklayın.
        </div>
      )
    )}
...
const RaporlamaAnalitik: React.FC = () => {
  ...
  const chartData = useMemo(() => {
    const aidRecipients = people.filter(
      p => p.aldigiYardimTuru && p.aldigiYardimTuru.length > 0
    );

    const nationalityCounts = aidRecipients.reduce(
      (acc, person) => {
    ...
    const nationalityData = Object.entries(nationalityCounts).map(
    ...
    const monthly = financials.reduce(
      (acc, record) => {
    ...
    const monthlyFinancialData = Object.values(monthly)
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
        ...
        <ChartCard title='Yardım Alanların Uyruk Dağılımı'>
          <ResponsiveContainer>
            <PieChart>
              <Pie
                data={chartData.nationalityData}
                dataKey='value'
                nameKey='name'
                cx='50%'
                cy='50%'
                ...
                label
              >
                {chartData.nationalityData.map((entry, index) => (
                  <Cell
                    key={`cell-${index}`}
