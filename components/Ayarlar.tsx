...
import {
  getSistemAyarlari,
  updateSistemAyarlari
} from '../services/apiService';
import type { SistemAyarlari } from '../types';
...
const Ayarlar: React.FC = () => {
  ...
  const FormCard: React.FC<{ title: string; children: React.ReactNode }> = ({
    title,
    children
  }) => (
  ...
  if (!ayarlar)
    return <div className='text-center py-10'>Ayarlar yüklenemedi.</div>;
        ...
        <FormRow label='Logo URL'>
          <div className='flex items-center space-x-4'>
            <img
              src={ayarlar.logoUrl}
              alt='Dernek Logosu'
              className='h-12 w-12 bg-slate-100 p-1 rounded-md object-contain'
            />
            <input
              type='text'
              name='logoUrl'
              value={ayarlar.logoUrl}
              onChange={handleChange}
              className='w-full px-3 py-2 border border-slate-300 rounded-lg'
            />
          </div>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
          <FormRow label='Varsayılan Para Birimi'>
            <select
              ...
              value={ayarlar.varsayilanParaBirimi}
              onChange={handleChange}
