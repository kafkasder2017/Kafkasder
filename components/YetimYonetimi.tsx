...
import type { Yetim } from '../types';
import { EgitimSeviyesi, DestekDurumu } from '../types';

import Modal from './Modal';

const getStatusClass = (status: DestekDurumu) => {
  return status === DestekDurumu.DESTEK_ALIYOR
  ...
  const filteredYetimler = useMemo(() => {
    return orphans
      .filter(yetim => {
        ...
        const matchesSearch =
          yetim.adiSoyadi.toLowerCase().includes(lowerSearch) ||
          yetim.veliAdi.toLowerCase().includes(lowerSearch);
        const matchesStatus =
          statusFilter === 'all' || yetim.destekDurumu === statusFilter;
        const matchesEducation =
          educationFilter === 'all' || yetim.egitimSeviyesi === educationFilter;
        ...
        <div className='flex flex-col md:flex-row items-center justify-between space-y-4 md:space-y-0 mb-6'>
          ...
          <div className='flex items-center space-x-4'>
            <select
              ...
              onChange={e =>
                setEducationFilter(e.target.value as EgitimSeviyesi | 'all')
              }
            >
              <option value='all'>Tüm Eğitim Seviyeleri</option>
              {Object.values(EgitimSeviyesi).map(seviye => (
            ...
            <select
              ...
              value={statusFilter}
              onChange={e =>
                setStatusFilter(e.target.value as DestekDurumu | 'all')
              }
            >
              <option value='all'>Tüm Destek Durumları</option>
              {Object.values(DestekDurumu).map(durum => (
            ...
            <tbody className='divide-y divide-slate-200'>
              {filteredYetimler.map(yetim => (
                <tr key={yetim.id} className='hover:bg-slate-50'>
                  ...
                  <td className='px-6 py-4'>
                    <span
                      className={`px-2.5 py-1 text-xs font-semibold rounded-full ${getStatusClass(yetim.destekDurumu)}`}
                    >
                      {yetim.destekDurumu}
                    </span>
                  </td>
                  <td className='px-6 py-4'>
                    {new Date(yetim.kayitTarihi).toLocaleDateString('tr-TR')}
                  </td>
                  <td className='px-6 py-4 text-right'>
                    <div className='flex items-center justify-end space-x-4'>
                      <ReactRouterDOM.Link
                        to={`/yetimler/${yetim.id}`}
                        className='text-blue-600 hover:text-blue-800 font-semibold'
                      >
                        Detay
                      </ReactRouterDOM.Link>
                      <button
                        onClick={() => handleEditClick(yetim)}
                        className='text-yellow-600 hover:text-yellow-800 font-semibold'
                      >
                        Düzenle
                      </button>
                      <button
                        onClick={() => handleDeleteClick(yetim.id)}
...
const YetimFormModal: React.FC<{
  yetim: Partial<Yetim>;
  onClose: () => void;
  onSave: (yetim: Partial<Yetim>) => void;
}> = ({ yetim, onClose, onSave }) => {
  ...
  return (
    <Modal
      isOpen={true}
      onClose={onClose}
      title={isNew ? 'Yeni Yetim Ekle' : 'Yetim Bilgilerini Düzenle'}
    >
